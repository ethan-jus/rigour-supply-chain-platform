...
本仓库已实现 IAM OIDC、Gateway 资源服务器、配置化基础管理、Integration 连接器基础能力、订单中心本地投影、Sales Work V1 领域库表骨架，以及内部协作 IM/会议服务骨架；尚未进入业务实现的领域服务仍是可编译骨架。自动构建通过不表示共享 DEV、浏览器或生产环境已验收。
...
└── rigour-<domain>-service/      # 13个领域服务聚合工程
...
- artifactId 唯一；
- 应用集合为一个 Gateway 加 13 个领域服务（含 Collaboration 协作服务）；
- 领域服务之间没有直接 Maven 依赖。

领域 API 模块只保存已确认的跨服务契约，未确认的接口和 DTO 不提前创建。IAM、Integration、订单中心、Sales Work 和 Collaboration 已分别具备已提交的 Schema/迁移；其余领域服务的空 DEV Schema/账号初始化见[`docs/DOMAIN_DATABASE_RUNTIME.md`](docs/DOMAIN_DATABASE_RUNTIME.md)，在各自字段级设计确认前不接入业务表、MyBatis-Plus 或 Flyway。Sales Work 的当前边界见 [`services/rigour-sales-work-service/README.md`](services/rigour-sales-work-service/README.md)，协作服务当前承载内部 IM、附件、设备推送登记和 LiveKit 会议令牌签发。
...
- 其余领域服务的数据库驱动、迁移和仓储实现；Sales Work 与 Collaboration 已有代码迁移和基础实现，但共享 DEV Flyway、真实设备推送、LiveKit/coturn 和移动端真机验收仍未完成；
...
IAM 当前检出代码的迁移目录为 V1～V50；V50 为门户工作台 App 补充 `workbench.*` 导航资源和 `collaboration:*` 权限，历史应用编码暂保持 `FEISHU_SALES` 兼容。共享 DEV 实际已执行版本、表数量和源码差异必须以[`docs/IAM_DATABASE_RUNTIME.md`](docs/IAM_DATABASE_RUNTIME.md)的最新只读核验为准，不能从代码目录推断运行时已发布。Integration 为独立的`rigour_integration` Schema；Sales Work 与 Collaboration 均为独立领域 Schema。其余领域库的 Schema 和账号初始化见[`docs/DOMAIN_DATABASE_RUNTIME.md`](docs/DOMAIN_DATABASE_RUNTIME.md)。
