...
DOMAINS = [
    ('settings', '公共基础', 'rg-scdp-foundation/foundation-server', 26892, 'settings', 'BUSINESS_SETTINGS'),
    ('hr', '人事薪酬', 'rg-scdp-hr/hr-server', 26889, 'hr', 'HR'),
    ('crm', '客户管理', 'rg-scdp-crm/crm-server', 26883, 'crm', 'CRM'),
    ('erp', 'ERP核心', 'rg-scdp-erp/erp-server', 26884, 'erp', 'ERP'),
    ('order', '订单中心', 'rg-scdp-order/order-server', 26885, 'order', 'ORDER'),
    ('sales', '销售工作', 'rg-scdp-sales/sales-server', 26886, 'sales_work', 'SALES_WORK'),
    ('ai', 'AI智能体', 'rg-scdp-ai/ai-server', 26887, 'ai', 'AI'),
    ('bi', '分析看板', 'rg-scdp-bi/bi-server', 26888, 'bi', 'BI'),
    ('integration', '集成迁移', 'rg-scdp-integration/integration-server', 26882, 'integration', 'INTEGRATION'),
]
CORE = ['iam', 'gateway', 'web']
PORTS = {'iam': 26881, 'gateway': 26880, **{s[0]: s[3] for s in DOMAINS}}


def select_services(value=None):
    """部分发布仍保留身份与SCDP 前端；未知名称直接拒绝，不静默漏部署。"""
...
def business_compose():
    ...
    for name, title, module, port, suffix, prefix in DOMAINS:
        jar = module.split('/')[-1] + '.jar'
        result[name] = {
            'image': 'rigour-desktop-' + name + ':${RELEASE_ID:?未设置发布版本}',
            'build': {'context': '.', 'dockerfile': 'Dockerfile.service', 'args': {'SERVICE_JAR': jar}},
            ...
            'command': ['java', '-jar', '/app/' + jar, '--spring.profiles.active=desktop',
