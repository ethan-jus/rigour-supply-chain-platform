...
2026-08-03 运行时快照：`rigour_iam` 为 V1～V14、35 张业务表；`rigour_integration` 为 V1～V2、13 张业务表；本次新增的 9 个领域库均为 0 张表。IAM 运行时比当前检出代码（迁移目录 V1～V11）多出 V12～V14，属于既有源码/环境漂移，本次不修改或回滚。
...
- 本次不创建业务表，也不向库中插入租户、订单、门店、员工或测试数据。各服务字段级设计冻结后，再添加对应服务的 Flyway V1。
...
当前 9 个服务还没有 JDBC/Flyway 依赖和正式业务迁移，因此不能仅凭创建空库就宣称服务已接入数据库或已可用。
