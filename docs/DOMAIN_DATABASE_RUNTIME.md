...
2026-08-06 18:52运行时快照：`rigour_iam` 已执行V1～V21，共35张表（34张业务表加Flyway历史表）；`rigour_integration`最近已确认的基线为V1～V2、13张业务表。Sales Work V1目前只在源码和隔离MySQL中验证；ERP 商品主数据 V1 已进入源码但尚未在共享 DEV 验证。两者是否已发布都必须以数据库 Flyway 历史只读核验为准，不能从源码推断。
