...
public class OrderInfrastructureConfiguration {
    ...
    ErpSalesStockOutClient erpSalesStockOutClient(
            ...
            SimpleClientHttpRequestFactory requestFactory, Environment environment) {
        return new HttpErpSalesStockOutClient(RestClient.builder().requestFactory(requestFactory), signer,
    ...
    OrderRepairUnitDictionary orderRepairUnitDictionary(
            TrustedContextSigner signer,
            @Value("${rigour.business-settings.base-url:http://localhost:26892}") String settingsBaseUrl,
            SimpleClientHttpRequestFactory requestFactory, Environment environment) {
        return new HttpOrderRepairUnitDictionary(RestClient.builder().requestFactory(requestFactory), signer,
                localServiceUrl(environment, settingsBaseUrl, "rigour-business-settings-service", 26892));
    }
    ...
    ErpOrderRepairCatalog erpOrderRepairCatalog(
            ...
            @Value("${rigour.erp.base-url:http://localhost:26884}") String erpBaseUrl,
            SimpleClientHttpRequestFactory requestFactory, Environment environment) {
        return new HttpErpOrderRepairCatalog(RestClient.builder().requestFactory(requestFactory), signer,
                localServiceUrl(environment, erpBaseUrl, "rigour-erp-core-service", 26884));
    }
    ...
    HrEmployeeDisplayClient hrEmployeeDisplayClient(
            ...
            SimpleClientHttpRequestFactory requestFactory, Environment environment) {
        return new HttpHrEmployeeDisplayClient(RestClient.builder().requestFactory(requestFactory), signer,
    ...
    CrmCustomerAreaDisplayClient crmCustomerAreaDisplayClient(
            ...
            SimpleClientHttpRequestFactory requestFactory, Environment environment) {
        return new HttpCrmCustomerAreaDisplayClient(RestClient.builder().requestFactory(requestFactory), signer,
    ...
    static String localServiceUrl(Environment environment, String configured, String serviceHost, int port) {
        if (!environment.acceptsProfiles(Profiles.of("local")) || configured == null) return configured;
        try {
            URI uri = new URI(configured);
            if (!serviceHost.equalsIgnoreCase(uri.getHost())) return configured;
            return new URI(uri.getScheme(), uri.getUserInfo(), "127.0.0.1", port,
