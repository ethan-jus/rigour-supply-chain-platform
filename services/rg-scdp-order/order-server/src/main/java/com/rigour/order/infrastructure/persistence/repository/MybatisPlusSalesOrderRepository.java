...
public class MybatisPlusSalesOrderRepository
    ...
    public MybatisPlusSalesOrderRepository(
            ...
            Clock orderClock,
            OrderAttributionWriter attribution,
    ...
    public OrderPageView<SalesOrderSummaryView> salesOrders(
        ...
        List<InternalSalesOrderEntity> rows =
                mapper.selectList(
        ...
        List<SalesOrderSummaryView> items =
                rows.stream().map(row -> summary(row, shipmentStatuses.get(row.getId()))).toList();
        return new OrderPageView<>(total, begin, step, items);
    }

    @Override
    public SalesOrderTotalsView salesOrderTotals(
            String tenantId, SalesOrderSearchCriteria criteria) {
        List<InternalSalesOrderEntity> rows =
                getBaseMapper()
                        .selectList(
                                query(tenantId, criteria)
        ...
        return new SalesOrderTotalsView(
                rows.size(),
                totalQuantity,
                originalAmount,
                discountAmount,
    ...
    public Optional<SalesOrderDetailView> salesOrder(String tenantId, Long id) {
        var visible =
                scopes.apply(
                        Wrappers.<InternalSalesOrderEntity>lambdaQuery()
    ...
    public boolean existsByNo(String tenantId, String orderNo) {
        return getBaseMapper()
                        .selectCount(
                                Wrappers.<InternalSalesOrderEntity>lambdaQuery()
                                        .eq(InternalSalesOrderEntity::getTenantId, tenantId)
                                        .eq(InternalSalesOrderEntity::getOrderNo, orderNo))
                > 0;
    }
    ...
    public SalesOrderDetailView create(
        ...
        try {
            ...
            attribution.prepare(
                    tenantId, entity.getId(), "SUBMITTED".equals(command.orderStatusCode()), true);
            scopes.requireOrder(tenantId, entity.getId(), "order:create");
            if ("SUBMITTED".equals(command.orderStatusCode()))
                scopes.requireOrder(tenantId, entity.getId(), "order:submit");
        } catch (DataIntegrityViolationException exception) {
            throw conflict("销售订单号已存在或销售订单引用数据无效");
        }
        return rawDetail(tenantId, entity.getId());
    }
    ...
    public SalesOrderDetailView update(
        ...
        scopes.requireOrder(tenantId, id, "order:update");
                ...
                getBaseMapper()
                        .update(
                                ...
                                Wrappers.<InternalSalesOrderEntity>lambdaUpdate()
                                        .set(
                                                InternalSalesOrderEntity::getSourceSystemCode,
                                                command.sourceSystemCode())
                                        .set(
                                                InternalSalesOrderEntity::getSourceOrderNo,
        ...
        attribution.prepare(tenantId, id, "SUBMITTED".equals(command.orderStatusCode()), true);
        if (!java.util.Objects.equals(existing.getCustomerId(), command.customerId()))
            scopes.requireAttributedOrder(tenantId, id, "order:update");
        scopes.requireOrder(tenantId, id, "order:update");
        if ("SUBMITTED".equals(command.orderStatusCode()))
            scopes.requireOrder(tenantId, id, "order:submit");
    ...
    public SalesOrderDetailView updateExternalProjection(
        ...
        BigDecimal sourceUnpaidAmount =
                command.sourceUnpaidAmount() == null
    ...
    public SalesOrderDetailView submit(String tenantId, Long id, int revision, String actorId) {
        ...
        attribution.prepare(tenantId, id, true, false);
        scopes.requireOrder(tenantId, id, "order:submit");
        LocalDateTime now = now();
        int updated =
                getBaseMapper()
    ...
    public SalesOrderDetailView cancel(String tenantId, Long id, int revision, String actorId) {
        attribution.requireNoExecution(tenantId, id);
        scopes.requireOrder(tenantId, id, "order:cancel");
        InternalSalesOrderEntity existing =
                selectActive(tenantId, id).orElseThrow(() -> notFound("销售订单不存在"));
                ...
                getBaseMapper()
                        .update(
                                ...
                                Wrappers.<InternalSalesOrderEntity>lambdaUpdate()
                                        .set(
                                                InternalSalesOrderEntity::getOrderStatusCode,
                                                SalesOrderStatus.CANCELLED.code())
                                        .set(
                                                InternalSalesOrderEntity::getPaymentStatusCode,
                                                SalesOrderPaymentStatus.CANCELLED.code())
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SalesOrderDetailView cancelBySource(
            String tenantId, Long id, int revision, String actorId) {
        attribution.requireNoExecution(tenantId, id);
        InternalSalesOrderEntity existing =
                selectActive(tenantId, id).orElseThrow(() -> notFound("销售订单不存在"));
        if (SalesOrderStatus.CANCELLED.code().equals(existing.getOrderStatusCode())
                ...
                && ZERO.compareTo(
                                existing.getUnpaidAmount() == null
                                        ? ZERO
                                        : existing.getUnpaidAmount())
                        == 0) {
            return rawDetail(tenantId, id);
        }
    ...
    public SalesOrderDetailView confirmOutbound(
        ...
        attribution.requireExecutedReceipt(tenantId, id);
        InternalSalesOrderEntity existing =
                selectActive(tenantId, id).orElseThrow(() -> notFound("销售订单不存在"));
        ...
        int updated =
                getBaseMapper()
                        .update(
                                ...
                                Wrappers.<InternalSalesOrderEntity>lambdaUpdate()
                                        .set(
                                                InternalSalesOrderEntity::getOutboundStatusCode,
                                                SalesOrderOutboundStatus.OUT_CONFIRMED.code())
                                        .set(
                                                InternalSalesOrderEntity::getShipmentTime,
                                                local(
                                                        firstNonNull(
                                                                shipmentTime,
                                                                instant(existing.getShipmentTime()),
                                                                instant(now))))
                                        .set(InternalSalesOrderEntity::getRevision, revision + 1)
                                        .set(
                                                InternalSalesOrderEntity::getUpdatedBy,
                                        ...
                                        .eq(
                                                InternalSalesOrderEntity::getOutboundStatusCode,
                                                SalesOrderOutboundStatus.PENDING.code())
                                        .eq(InternalSalesOrderEntity::getDeleted, 0));
        if (updated != 1) throw conflict("销售订单已被其他人修改，请刷新后重试");
        return rawDetail(tenantId, id);
    }
    ...
    public void delete(String tenantId, Long id, int revision, String actorId) {
        attribution.requireNoExecution(tenantId, id);
        scopes.requireOrder(tenantId, id, "order:delete");
        requireDraft(tenantId, id);
        LocalDateTime now = now();
        int updated =
                getBaseMapper()
    ...
    private SalesOrderDetailView rawDetail(String tenantId, long id) {
        return selectActive(tenantId, id)
                .map(o -> detail(o, lines(tenantId, id), shipmentStatus(tenantId, id)))
    ...
    private LambdaQueryWrapper<InternalSalesOrderEntity> query(
            String tenantId, SalesOrderSearchCriteria criteria) {
        LambdaQueryWrapper<InternalSalesOrderEntity> query =
                Wrappers.<InternalSalesOrderEntity>lambdaQuery()
                        .eq(InternalSalesOrderEntity::getTenantId, tenantId)
                        .eq(InternalSalesOrderEntity::getDeleted, 0);
        scopes.apply(query, "order:read");
        if (criteria.orderNo() != null)
            query.like(InternalSalesOrderEntity::getOrderNo, criteria.orderNo());
        ...
        if (criteria.dataQualityStatusCode() != null) {
            query.eq(
                    InternalSalesOrderEntity::getDataQualityStatusCode,
                    criteria.dataQualityStatusCode());
        }
        ...
        if (criteria.regionCode() != null)
            query.eq(InternalSalesOrderEntity::getRegionCode, criteria.regionCode());
        ...
        if (criteria.outboundStatusCode() != null) {
            query.eq(
                    InternalSalesOrderEntity::getOutboundStatusCode, criteria.outboundStatusCode());
        }
    ...
    private List<Long> matchingLineOrderIds(String tenantId, SalesOrderSearchCriteria criteria) {
        LambdaQueryWrapper<InternalSalesOrderLineEntity> query =
                Wrappers.<InternalSalesOrderLineEntity>lambdaQuery()
        ...
        if (criteria.productVariantId() != null) {
            query.eq(
                    InternalSalesOrderLineEntity::getProductVariantId, criteria.productVariantId());
        }
        if (criteria.productCodeSnapshot() != null) {
            query.eq(
                    InternalSalesOrderLineEntity::getProductCodeSnapshot,
                    criteria.productCodeSnapshot());
        }
        ...
        if (criteria.productNameSnapshot() != null) {
            query.like(
                    InternalSalesOrderLineEntity::getProductNameSnapshot,
                    criteria.productNameSnapshot());
        }
        if (criteria.specificationSnapshot() != null) {
            query.like(
                    InternalSalesOrderLineEntity::getSpecificationSnapshot,
                    criteria.specificationSnapshot());
        }
    ...
    private void logicDeleteLines(
        ...
        lineMapper.update(
                null,
                Wrappers.<InternalSalesOrderLineEntity>lambdaUpdate()
    ...
    private Map<Long, String> shipmentStatuses(
            String tenantId, List<InternalSalesOrderEntity> orders) {
        ...
        for (InternalSalesShipmentEntity shipment :
    ...
    private static SalesOrderSummaryView summary(
            InternalSalesOrderEntity entity, String shipmentStatusCode) {
        return new SalesOrderSummaryView(
                entity.getId(),
                entity.getOrderNo(),
                entity.getSourceSystemCode(),
                entity.getSourceOrderNo(),
                entity.getSourceStatusCode(),
                entity.getSourceCreatorId(),
                entity.getSourceCreatorStaffCode(),
                entity.getSourceCreatorName(),
                entity.getDataQualityStatusCode(),
                entity.getDataQualityMessage(),
                ...
                instant(entity.getPaymentTime()),
                instant(entity.getShipmentTime()),
                shipmentStatusCode,
                entity.getOrderStatusCode(),
                entity.getPaymentStatusCode(),
                entity.getOutboundStatusCode(),
    ...
    private static SalesOrderDetailView detail(
            InternalSalesOrderEntity entity,
        ...
        return new SalesOrderDetailView(
                ...
                entity.getUpdatedBy(),
                instant(entity.getUpdatedTime()),
                parseStrings(entity.getPaymentVoucherKeysJson()),
                List.of(),
                lines);
    }
    ...
    private static BigDecimal totalDiscountAmount(InternalSalesOrderEntity entity) {
        BigDecimal derived =
                amount(entity.getOriginalAmount()).subtract(amount(entity.getPayableAmount()));
    ...
    private static BigDecimal initialUnpaidAmount(SalesOrderWrite command) {
        ...
        return unpaidAmount(
                command.sourceSystemCode(),
                false,
                amount(command.payableAmount()),
                ZERO,
                command.sourceUnpaidAmount());
    }

    private static BigDecimal unpaidAmount(
    ...
    private static boolean sourceProjectionSame(
        ...
        return Objects.equals(existing.getSourceStatusCode(), command.sourceStatusCode())
                ...
                && Objects.equals(
                        existing.getSourceCreatorStaffCode(), command.sourceCreatorStaffCode())
