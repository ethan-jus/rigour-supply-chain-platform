package com.rigour.order.maintenance;
 ...
 * {@code --rigour.order.maintenance=flyway-repair} 时，服务才会在 Web 容器启动前执行
 * {@link Flyway#repair()} 和 {@link Flyway#migrate()}，随后直接退出。</p>
 *
 * <p>该模式用于共享 DEV 或开发环境修复失败迁移记录。它只读取环境变量中的迁移账号密码，不输出密码。</p>
 */
public final class OrderFlywayMaintenance {

    public static final String ARGUMENT = "--rigour.order.maintenance=flyway-repair";
    private static final String DEFAULT_JDBC_URL = "jdbc:mysql://82.157.4.176:13306/rigour_order"
    ...
    private static final String DEFAULT_USER = "rigour_order_migrator";

    private OrderFlywayMaintenance() {
    }
    ...
    private static void run() {
        String url = env("ORDER_DB_MIGRATOR_URL", DEFAULT_JDBC_URL);
        String user = env("ORDER_DB_MIGRATOR_USER", DEFAULT_USER);
        String password = requiredEnv("ORDER_DB_MIGRATOR_PASSWORD");

        System.out.println("Order Flyway 维护模式启动");
        ...
        System.out.println("Order Flyway 维护模式结束，请移除维护参数后正常启动服务");
    }
    ...
    private static String requiredEnv(String name) {
        ...
        if (value == null || value.isBlank()) {
            throw new IllegalStateException("缺少环境变量 " + name + "，请在 Order 服务 Run Configuration 中配置迁移密码");
        }
        return value;
    }
