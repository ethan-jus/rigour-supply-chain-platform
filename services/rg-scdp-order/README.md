...
Order Center 不实现订货宝 `f/v` 协议，不保存外部 Secret/Token；通过 `integration-api` 的版本化查询契约调用 Integration。当前工作区已确认可调用订单列表和订单详情；同步结果使用 `tenantId + sourceSystem + sourceOrderNo` 做幂等。
...
- 允许依赖：自己的 `order-api`、平台 shared 模块和经过评审的消息/API 客户端。
- 禁止依赖：`integration-migration-service` 实现模块、其他领域服务实现模块、跨库 SQL。
- 跨服务协作使用 `order-api` 的版本化契约、内部事件或受控本地投影。

## 启动与验证

```bash
./mvnw -B -pl services/rg-scdp-order/order-server -am test
```
