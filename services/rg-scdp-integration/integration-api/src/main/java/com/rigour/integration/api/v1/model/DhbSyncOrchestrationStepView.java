...
public record DhbSyncOrchestrationStepView(
        ...
        /** 失败或跳过原因；成功时为空。 */
        String message,
        /** 明细为空表示该领域未提供此口径，不能按零展示。 */
        Long created,
        Long updated,
        Long repaired,
        Long duplicates,
        Long rejected) {
    public DhbSyncOrchestrationStepView(String domain, String objectType, String status, UUID runId,
            long fetched, long changed, long unmapped, Map<String, Long> dictionaryRevisions, String message) {
        this(domain, objectType, status, runId, fetched, changed, unmapped, dictionaryRevisions, message,
