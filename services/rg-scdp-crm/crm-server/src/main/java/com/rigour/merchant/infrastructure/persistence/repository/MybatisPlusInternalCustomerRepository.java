...
public class MybatisPlusInternalCustomerRepository
    ...
    private final com.rigour.merchant.application.port.out.CustomerShippingAddressStore shipping;
    private final CrmDataScope scopes;
    private final CrmResponsibilityWriter responsibility;
    private final CustomerAssignmentScope assignmentTargets;
    private final org.springframework.transaction.support.TransactionTemplate transaction;

    public MybatisPlusInternalCustomerRepository(
            InternalCustomerMapper mapper,
            ...
            Clock crmClock,
            com.rigour.merchant.application.port.out.CustomerShippingAddressStore shipping,
            CrmDataScope scopes,
            CrmResponsibilityWriter responsibility,
            CustomerAssignmentScope assignmentTargets,
            org.springframework.transaction.PlatformTransactionManager manager, CustomerIdentityGuard identities) {
    ...
    public PageView<InternalCustomerSummaryView> customers(
            String tenantId, int begin, int step, CustomerSearchCriteria criteria) {
    ...
    @Override
    @Transactional
    public InternalCustomerDetailView create(
            String tenantId, String customerCode, InternalCustomerCommand command, String actorId) {
        command = canonicalOwner(tenantId, command, null);
        scopes.requireProposed(
                tenantId, command.ownerEmployeeCode(), command.regionCode(), "crm:customer:create");
        String identityConflict = identities.conflict(tenantId, null, command.customerName(), command.loginAccount(), null, command.regionCode());
        if (identityConflict != null) throw conflict(identityConflict);
        LocalDateTime now = now();
        InternalCustomerEntity entity = new InternalCustomerEntity();
        entity.setOwnerManagementMode("LOCAL");
        entity.setRegionManagementMode("LOCAL");
        entity.setTenantId(tenantId);
        entity.setCustomerCode(customerCode);
        entity.setCustomerName(command.customerName());
        entity.setLoginAccount(command.loginAccount());
        ...
        entity.setBusinessCreatedAt(now);
        entity.setBusinessCreatedById(actorId);
        entity.setBusinessCreationSource("INTERNAL");
        ...
        responsibility.changed(
                ...
                "新建客户",
                true);
        shipping.createInitial(tenantId, entity.getId(), command.shippingAddress(), actorId);
        return detail(requireActive(tenantId, entity.getId()));
    }

    @Override
    @Transactional
    public InternalCustomerDetailView update(
            String tenantId, Long id, InternalCustomerCommand command, String actorId) {
        scopes.requireCustomer(tenantId, id, "crm:customer:update");
        InternalCustomerEntity existing = requireActive(tenantId, id);
        if (!Objects.equals(existing.getOwnerEmployeeCode(), command.ownerEmployeeCode())
                || !Objects.equals(existing.getRegionCode(), command.regionCode()))
            throw new IllegalArgumentException("请通过客户归属管理调整主责或地区，并填写变更原因");
        String identityConflict = identities.conflict(tenantId, id, command.customerName(), command.loginAccount(), existing.getCityName(), command.regionCode());
        if (identityConflict != null) throw conflict(identityConflict);
        command = canonicalOwner(tenantId, command, existing);
        scopes.requireProposed(
                tenantId, command.ownerEmployeeCode(), command.regionCode(), "crm:customer:update");
        LocalDateTime now = now();
        int updated =
                getBaseMapper()
                        .update(
                                ...
                                Wrappers.<InternalCustomerEntity>lambdaUpdate()
                                        .set(
                                                ...
                                                InternalCustomerEntity::getOwnerManagementMode,
                                                "LOCAL")
                                        .set(
                                                !Objects.equals(
                                                        existing.getRegionCode(),
                                                        command.regionCode()),
                                                InternalCustomerEntity::getRegionManagementMode,
                                                "LOCAL")
                                        .set(InternalCustomerEntity::getLoginAccount, command.loginAccount())
                                        .set(
                                                InternalCustomerEntity::getCustomerName,
                                                command.customerName())
                                        .set(
                                                InternalCustomerEntity::getContactName,
                                                command.contactName())
                                        .set(
                                                InternalCustomerEntity::getContactPhone,
                                                command.contactPhone())
                                        .set(
                                                InternalCustomerEntity::getCustomerTypeCode,
                                                command.customerTypeCode())
                                        .set(
                                                InternalCustomerEntity::getRegionCode,
                                                command.regionCode())
                                        .set(
                                        ...
                                        .set(
                                                InternalCustomerEntity::getSettlementTypeCode,
                                                command.settlementTypeCode())
                                        .set(InternalCustomerEntity::getAddress, command.address())
                                        .set(
                                                InternalCustomerEntity::getStatusCode,
                                                command.statusCode())
    ...
    public void delete(String tenantId, Long id, int revision, String actorId) {
        scopes.requireCustomer(tenantId, id, "crm:customer:delete");
    ...
    @Override
    public ExternalCrmCustomerSyncResult syncExternalCustomers(
            String tenantId,
            String sourceSystem,
        ...
        int unchanged = 0;
        int failed = 0;
        for (ExternalCrmCustomerRowCommand row :
                rows == null ? List.<ExternalCrmCustomerRowCommand>of() : rows) {
            try {
                SyncOutcome outcome =
                                ...
                                transaction.execute(
                                        status ->
                                                syncExternalRow(
                                                        ...
                                                        codeGenerator)));
                rowResults.add(
                        new ExternalCrmCustomerSyncRowResult(
                                outcome.sourceCustomerId(),
                                outcome.customerId(),
                ...
                rowResults.add(
                        new ExternalCrmCustomerSyncRowResult(
                                row == null ? null : row.sourceCustomerId(),
                                null,
                                null,
                                "FAILED",
                                message));
    ...
    private SyncOutcome syncExternalRow(
            String tenantId,
            String sourceSystem,
        ...
        InternalCustomerEntity existing =
                customerBySource(tenantId, sourceSystem, sourceTenantKey, row.sourceCustomerId());
        // 已与订货宝确认对应后，飞书历史订单重放不能倒灌当前客户归属或改名。
        if (existing != null && "FEISHU".equals(sourceSystem) && existing.getPartyId() != null
                && getBaseMapper().countDhbBindings(tenantId, existing.getPartyId()) > 0)
            return new SyncOutcome(row.sourceCustomerId(), existing.getId(), existing.getCustomerCode(),
                    "UNCHANGED", "已采用订货宝当前客户资料，保留飞书历史建档事实");
        String identityConflict = identities.conflict(tenantId, existing == null ? null : existing.getId(), row.customerName(),
                existing == null ? null : existing.getLoginAccount(), row.cityName(), existing == null ? null : existing.getRegionCode());
        if (identityConflict != null) throw conflict(identityConflict);
        ...
        String regionCode =
                resolveCustomerAreaCode(tenantId, sourceSystem, row, actorId, codeGenerator, now);
        if (existing == null) {
            String customerCode =
                    codeGenerator.generateUnique(
                            CrmBusinessCodeRules.CUSTOMER,
                            row.sourceCreatedAt(),
                            candidate -> !existsByCode(tenantId, candidate));
            PartySync partySync =
                    ensureParty(tenantUuid, sourceSystem, null, customerCode, row, actorId, now);
            InternalCustomerEntity entity = new InternalCustomerEntity();
            entity.setTenantId(tenantId);
            entity.setCustomerCode(customerCode);
            entity.setPartyId(partySync.entity().id);
            copyExternalCustomerFields(
                    entity, sourceSystem, sourceTenantKey, row, customerTypeCode, regionCode);
            entity.setRevision(1);
            entity.setCreatedBy(actorId);
            entity.setCreatedTime(now);
            entity.setUpdatedBy(actorId);
            ...
            responsibility.changed(
                    ...
                    "导入客户主责及归属地区",
                    true);
            upsertCustomerDirectoryProjection(
                    tenantUuid,
                    partySync.entity().id,
                    customerType,
                    regionCode,
        ...
        String previousOwnerCode = existing.getOwnerEmployeeCode(),
                previousOwnerName = existing.getOwnerEmployeeNameSnapshot();
        if (("LOCAL".equals(existing.getOwnerManagementMode())
                        && !Objects.equals(previousOwnerCode, clean(row.ownerEmployeeCode(), 50)))
                || ("LOCAL".equals(existing.getRegionManagementMode())
                        && !Objects.equals(previousRegionCode, regionCode)))
            responsibility.sourceConflict(
                    tenantId,
                    existing.getId(),
                    sourceSystem,
                    clean(row.ownerEmployeeCode(), 50),
                    regionCode,
                    row.sourcePayloadHash());
        boolean sourceUnchanged =
                sameHash(existing.getSourcePayloadHash(), row.sourcePayloadHash());
        copyExternalCustomerFields(
                existing, sourceSystem, sourceTenantKey, row, customerTypeCode, regionCode);
        existing.setPartyId(partySync.entity().id);
        boolean customerNeedsUpdate =
                !sourceUnchanged
                ...
                getBaseMapper()
                        .update(
                                ...
                                Wrappers.<InternalCustomerEntity>lambdaUpdate()
                                        ...
                                        .set(
                                                InternalCustomerEntity::getPartyId,
                                                existing.getPartyId())
                                        .set(
                                                InternalCustomerEntity::getContactName,
                                        ...
                                        .set(
                                                InternalCustomerEntity::getCustomerSourceName,
                                                existing.getCustomerSourceName())
                                        .set(
                                                InternalCustomerEntity::getBusinessCategoryName,
                                                existing.getBusinessCategoryName())
                                        .set(
                                                InternalCustomerEntity::getRegionCode,
                                                existing.getRegionCode())
                                        .set(
                                                InternalCustomerEntity::getRegionName,
                                                existing.getRegionName())
                                        .set(
                                                InternalCustomerEntity::getCityName,
                                                existing.getCityName())
                                        .set(
                                                InternalCustomerEntity::getOwnerEmployeeCode,
                                                existing.getOwnerEmployeeCode())
                                        .set(
                                        ...
                                        .set(
                                                InternalCustomerEntity::getSettlementTypeCode,
                                                existing.getSettlementTypeCode())
                                        .set(
                                                InternalCustomerEntity::getAddress,
                                                existing.getAddress())
                                        .set(
                                                InternalCustomerEntity::getStatusCode,
                                                existing.getStatusCode())
                                        .set(
                                                InternalCustomerEntity::getRemark,
                                                existing.getRemark())
                                        .set(
                                                InternalCustomerEntity::getSourceSystemCode,
                                                existing.getSourceSystemCode())
                                        .set(
                                                InternalCustomerEntity::getSourceTenantKey,
                                                existing.getSourceTenantKey())
                                        .set(
                                                InternalCustomerEntity::getSourceCustomerId,
                                                existing.getSourceCustomerId())
                                        .set(
                                                InternalCustomerEntity::getSourceDocumentNo,
                                                existing.getSourceDocumentNo())
                                        .set(
                                                InternalCustomerEntity::getSourceCreatedAt,
                                                existing.getSourceCreatedAt())
                                        .set(
                                                InternalCustomerEntity::getSourceUpdatedAt,
                                                existing.getSourceUpdatedAt())
                                        .set(
                                                InternalCustomerEntity::getSourcePayloadHash,
                                                existing.getSourcePayloadHash())
                                        .set(
                                                InternalCustomerEntity::getSourcePayloadJson,
                                                existing.getSourcePayloadJson())
        ...
        return new SyncOutcome(
                ...
                existing.getCustomerCode(),
                "UPDATED",
                sourceUnchanged ? "CRM客户关联主数据已补齐" : "CRM客户已更新");
    }

    private InternalCustomerCommand canonicalOwner(
            String tenant, InternalCustomerCommand c, InternalCustomerEntity old) {
        String code = c.ownerEmployeeCode(),
                name = old == null ? null : old.getOwnerEmployeeNameSnapshot();
        if (!Objects.equals(old == null ? null : old.getOwnerEmployeeCode(), code)) {
            responsibility.requireAssignmentPermission();
            scopes.requireProposed(tenant, code, c.regionCode(), "crm:customer:assign-owner");
            var owner = responsibility.requireOwner(tenant, code);
            assignmentTargets.requireEmployeeRegion(tenant, code, c.regionCode());
            name = owner == null ? null : owner.name();
        }
        return new InternalCustomerCommand(
                ...
                code,
                name,
                c.settlementTypeCode(),
                c.address(),
                c.statusCode(),
                c.remark(),
                c.revision(), c.loginAccount(), c.shippingAddress());
    }
    ...
    @Override
    public java.util.List<String> creators(String tenantId) {
        var q = Wrappers.<InternalCustomerEntity>lambdaQuery()
                .select(InternalCustomerEntity::getBusinessCreatedByName)
                .eq(InternalCustomerEntity::getTenantId, tenantId).eq(InternalCustomerEntity::getDeleted, 0)
                .isNotNull(InternalCustomerEntity::getBusinessCreatedByName)
                .ne(InternalCustomerEntity::getBusinessCreatedByName, "")
                .groupBy(InternalCustomerEntity::getBusinessCreatedByName)
                .orderByAsc(InternalCustomerEntity::getBusinessCreatedByName);
        return getBaseMapper().selectList(scopes.apply(q, "crm:customer:read")).stream()
                .map(InternalCustomerEntity::getBusinessCreatedByName).toList();
    }

    private LambdaQueryWrapper<InternalCustomerEntity> query(
            String tenantId, CustomerSearchCriteria criteria) {
        LambdaQueryWrapper<InternalCustomerEntity> query =
                Wrappers.<InternalCustomerEntity>lambdaQuery()
                        .eq(InternalCustomerEntity::getTenantId, tenantId)
                        .eq(InternalCustomerEntity::getDeleted, 0);
        ...
        if (criteria.createdFrom() != null) query.ge(InternalCustomerEntity::getBusinessCreatedAt,
                java.time.LocalDateTime.ofInstant(criteria.createdFrom().atStartOfDay(zone).toInstant(), java.time.ZoneOffset.UTC));
        if (criteria.createdTo() != null) query.lt(InternalCustomerEntity::getBusinessCreatedAt,
                java.time.LocalDateTime.ofInstant(criteria.createdTo().plusDays(1).atStartOfDay(zone).toInstant(), java.time.ZoneOffset.UTC));
        ...
        if (criteria.regionCode() != null) {
            query.in(InternalCustomerEntity::getRegionCode, regionAndDescendants(tenantId, criteria.regionCode()));
        }
    ...
    private java.util.Set<String> regionAndDescendants(String tenantId, String code) {
        var rows = customerAreaMapper.selectList(Wrappers.<CustomerAreaEntity>query()
                .eq("tenant_id", CrmUuidCodec.encode(UUID.fromString(tenantId))).eq("deleted", 0));
        var codes = new java.util.LinkedHashSet<String>(); codes.add(code);
        var queue = new java.util.ArrayDeque<String>(); queue.add(code);
        while (!queue.isEmpty()) {
            String parent = queue.removeFirst();
            for (var row : rows) if (parent.equals(row.parentAreaCode) && codes.add(row.areaCode)) queue.add(row.areaCode);
        }
        return codes;
    }

    private InternalCustomerEntity customerBySource(
            String tenantId, String sourceSystem, String sourceTenantKey, String sourceCustomerId) {
        return getBaseMapper()
                .selectOne(
                        Wrappers.<InternalCustomerEntity>lambdaQuery()
                                .eq(InternalCustomerEntity::getTenantId, tenantId)
    ...
    private void copyExternalCustomerFields(
            InternalCustomerEntity entity,
            String sourceSystem,
        ...
        if (!"LOCAL".equals(entity.getRegionManagementMode())) {
        ...
        // 订单派生的导入记录不能覆盖已核实的门店建档时间。
        if (!"FEISHU".equals(entity.getBusinessCreationSource()))
    ...
    private CustomerTypeEntity ensureCustomerType(
            UUID tenantId,
            String sourceSystem,
        ...
        if (existing != null) {
            ...
            var update =
                    Wrappers.<PartyEntity>update()
                            ...
                            .eq("id", existing.id);
            if (displayName != null
                    && !Objects.equals(existing.displayName, displayName)
                    && !"INTERNAL_PRIMARY".equals(existing.ownershipState)) {
    ...
    private boolean upsertCustomerDirectoryProjection(
            UUID tenantId,
            byte[] partyId,
            CustomerTypeEntity customerType,
            String regionCode,
    ...
    private boolean ensurePartyRole(
            UUID tenantId,
            byte[] partyId,
            Instant sourceCreatedAt,
            String actorId,
            LocalDateTime now) {
        PartyRoleEntity existing =
                partyRoleMapper.selectOne(
                        Wrappers.<PartyRoleEntity>query()
                                .eq("tenant_id", bytes(tenantId))
                                .eq("party_id", partyId)
        ...
        if (!"ACTIVE".equals(existing.status) || !Objects.equals(existing.deleted, 0)) {
            partyRoleMapper.update(
                    null,
                    Wrappers.<PartyRoleEntity>update()
                            .eq("tenant_id", bytes(tenantId))
                            .eq("party_id", partyId)
                            .eq("role_code", "CUSTOMER")
                            .set("status", "ACTIVE")
    ...
    private boolean upsertCustomerProfile(
            UUID tenantId,
            byte[] partyId,
            CustomerTypeEntity customerType,
            CustomerAreaEntity area,
        ...
        String typeName =
                customerType == null
                        ? clean(row.businessCategoryName(), 160)
                        : customerType.typeName;
        String areaName =
                area == null ? firstText(row.cityName(), row.regionName()) : area.areaName;
        ...
        boolean changed =
                !sameBytes(existing.customerTypeId, customerTypeId)
    ...
    private ContactSync upsertContact(
            UUID tenantId,
            byte[] partyId,
            String sourceSystem,
            String contactType,
        ...
        boolean changed =
                !Objects.equals(existing.contactName, name)
    ...
    private boolean upsertShippingAddress(
            UUID tenantId,
            byte[] partyId,
            String sourceSystem,
            ExternalCrmCustomerRowCommand row,
        ...
        ContactSync contact =
                upsertContact(
                        ...
                        "SHIPPING",
                        false,
                        firstText(row.contactName(), row.customerName()),
                        row.contactPhone(),
                        actorId,
                        now);
        ...
        boolean changed =
                contact.changed()
                        || !sameBytes(
                                existing.contactId,
                                contact.entity() == null ? null : contact.entity().id)
    ...
    private String resolveCustomerAreaCode(
            String tenantId,
            String sourceSystem,
            ExternalCrmCustomerRowCommand row,
            String actorId,
    ...
    private CustomerAreaEntity ensureArea(
            UUID tenantId,
            String sourceSystem,
    ...
    private static String meaningfulAddress(String sourceAddress) {
        ...
        String compact =
                address.replaceAll("(?m)(收件人|联系人|联系电话|联系方式|电话|地址)[:：]\\s*", "")
                        .replaceAll("[\\s\\r\\n]+", "");
    ...
    private static InternalCustomerSummaryView summary(InternalCustomerEntity entity) {
        return new InternalCustomerSummaryView(
                entity.getId(),
                entity.getCustomerCode(),
                ...
                entity.getCityName(),
                entity.getCustomerSourceName(),
                entity.getBusinessCategoryName(),
                entity.getOwnerSalesUserId(),
                entity.getOwnerSalesName(),
                entity.getOwnerEmployeeCode(),
                entity.getOwnerEmployeeNameSnapshot(),
                entity.getSettlementTypeCode(),
                entity.getStatusCode(),
                entity.getSourceSystemCode(),
                entity.getSourceDocumentNo(),
                instant(entity.getSourceCreatedAt()),
                instant(entity.getSourceUpdatedAt()),
                entity.getRevision(),
    ...
    private static InternalCustomerDetailView detail(InternalCustomerEntity entity) {
        return new InternalCustomerDetailView(
                entity.getId(),
                entity.getCustomerCode(),
                entity.getCustomerName(),
                entity.getContactName(),
