...
/** SCDP Web CRM 查询用例；所有页面只读取 CRM 本地表。 */
@Service
public final class CrmCustomerQueryService {
    ...
    public DictionaryView updateCustomerArea(UUID id, CrmCustomerAreaCommand command) {
        ...
        CallerIdentity actor = actor(WRITE_PERMISSION);
        return store.updateCustomerArea(
                actor.tenantId(), id, normalizeArea(command, true), actor.principalId());
    }
    ...
    public ExternalCrmAreaSyncResult syncExternalCustomerAreas(ExternalCrmAreaSyncCommand command) {
        ...
        ExternalCrmAreaSyncResult result =
                store.syncExternalCustomerAreas(
    ...
    private static CrmCustomerAreaCommand normalizeArea(
    ...
    private static List<ExternalCrmAreaRowCommand> externalAreaRows(
            List<ExternalCrmAreaRowCommand> rows) {
        List<ExternalCrmAreaRowCommand> source = rows == null ? List.of() : rows;
        if (source.size() > MAX_AREA_SYNC_ROWS)
            throw badRequest("地区同步单次不能超过" + MAX_AREA_SYNC_ROWS + "行");
