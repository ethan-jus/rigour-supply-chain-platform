...
import com.rigour.merchant.infrastructure.persistence.repository.CrmDataScope.MapperPredicate;
...
/** SCDP Web CRM 列表和详情的定向查询；分页和租户过滤均在数据库完成。 */
public interface CrmQueryMapper {
    ...
    @Select(
            ...
            SELECT COUNT(*)
             ...
             WHERE p.tenant_id=#{tenantId} AND p.deleted=0 AND ${scope.sql}
    ...
    long countCustomers(
            ...
            @Param("scope") MapperPredicate scope);
...
<if test="status != null and status != ''"> AND p.internal_status=#{status}</if>
 ORDER BY b.source_updated_at DESC,p.updated_time DESC
    ...
    @Select(
            ...
            SELECT sa.party_id,sa.assignment_type,sa.source_staff_id,sa.employee_code,
             ...
             ORDER BY sa.party_id,FIELD(sa.assignment_type,'PRIMARY','SECONDARY','SERVICE'),
                      sa.effective_from,sa.created_time
            </script>
            """)
    List<Map<String, Object>> salesAssignments(
            @Param("tenantId") byte[] tenantId, @Param("partyIds") List<byte[]> partyIds);
...
SELECT p.id,p.party_code,p.display_name,p.internal_status,cp.login_account,
  ...
  LEFT JOIN crm_source_binding b ON b.tenant_id=p.tenant_id AND b.target_id=p.id
       AND b.target_type='PARTY' AND b.source_object_type='CUSTOMER'
 WHERE p.tenant_id=#{tenantId} AND p.id=#{id} AND p.deleted=0 AND ${scope.sql}
 LIMIT 1
</script>
""")
    ...
    @Select(
            ...
            SELECT a.id,a.consignee,c.contact_name,c.phone,a.region_text,a.area_name,
             ...
             WHERE a.tenant_id=#{tenantId} AND a.party_id=#{partyId}
               AND a.address_type='SHIPPING' AND a.deleted=0
             ORDER BY a.is_default DESC,a.updated_time DESC
            """)
    List<Map<String, Object>> shippingAddresses(
            @Param("tenantId") byte[] tenantId, @Param("partyId") byte[] partyId);
    ...
    @Select(
            ...
            SELECT COUNT(*)
             ...
             WHERE a.tenant_id=#{tenantId} AND a.address_type='SHIPPING'
               AND a.deleted=0 AND p.deleted=0 AND ${scope.sql}
            <if test="query != null and query != ''">
               AND (p.party_code LIKE #{query} OR p.display_name LIKE #{query}
    ...
    @Select(
            ...
            <if test="query != null and query != ''">
             ...
             ORDER BY a.is_default DESC,b.source_updated_at DESC,a.updated_time DESC
             LIMIT #{step} OFFSET #{begin}
            </script>
            """)
    ...
    @Select(
            ...
            """)
    List<Map<String, Object>> customerTypes(
            @Param("tenantId") byte[] tenantId,
            @Param("begin") int begin,
            @Param("step") int step,
            @Param("query") String query);
    ...
    @Select(
            ...
            <if test="query != null and query != ''">
             ...
             ORDER BY d.sort_order,d.area_code LIMIT #{step} OFFSET #{begin}
            </script>
            """)
    List<Map<String, Object>> customerAreas(
            @Param("tenantId") byte[] tenantId,
            @Param("begin") int begin,
            @Param("step") int step,
            @Param("query") String query);
