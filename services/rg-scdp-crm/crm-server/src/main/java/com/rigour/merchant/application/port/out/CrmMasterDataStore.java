...
public interface CrmMasterDataStore {
    ...
    default List<SourceRecord> pendingCustomerRecords(UUID tenantId, UUID connectorId) { return List.of(); }

    default java.time.Instant customerSyncCursor(UUID tenantId, UUID connectorId) { return null; }

    default RunStatistics completeCustomerWindow(UUID tenantId, UUID connectorId, UUID runId,
            RunStatistics statistics, java.time.Instant watermark) {
        throw new UnsupportedOperationException("客户增量游标尚未实现");
    }
