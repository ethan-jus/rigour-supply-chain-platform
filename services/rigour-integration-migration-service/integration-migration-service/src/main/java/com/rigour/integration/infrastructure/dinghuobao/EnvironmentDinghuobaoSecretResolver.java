 ...
 * 开发环境的 Secret 引用适配器。
 *
 * <p>仅支持 {@code env://PREFIX}，实际值从进程环境的
 * {@code PREFIX_SERIAL_NUMBER} 和 {@code PREFIX_PASSWORD} 读取。生产环境应替换为
 * Vault/KMS 等 Secret Manager 实现；代码和 Nacos 只保存引用。</p>
 */
public final class EnvironmentDinghuobaoSecretResolver implements DinghuobaoSecretResolver {

    private final Function<String, String> valueSource;

    public EnvironmentDinghuobaoSecretResolver(Environment environment) {
        this(environment::getProperty);
    }

    EnvironmentDinghuobaoSecretResolver(Function<String, String> valueSource) {
    ...
    public Credentials resolve(String secretRef) {
        ...
        if (!ref.startsWith("env://") || ref.length() <= "env://".length()) {
            throw new DinghuobaoClientException(
                    "DINGHUOBAO_SECRET_REF_UNSUPPORTED",
        ...
        if (!prefix.matches("[A-Za-z_][A-Za-z0-9_]*")) {
            throw new DinghuobaoClientException(
                    "DINGHUOBAO_SECRET_REF_INVALID", "订货宝 Secret 引用格式无效", false, null, null);
        }
        ...
        if (serialNumber == null || serialNumber.isBlank() || password == null || password.isBlank()) {
            throw new DinghuobaoClientException(
                    "DINGHUOBAO_SECRET_NOT_CONFIGURED",
        ...
        try {
            ...
            throw new DinghuobaoClientException(
                    "DINGHUOBAO_SECRET_NOT_CONFIGURED", "订货宝 Secret 尚未配置完整", false, null, null);
        }
    }
}
