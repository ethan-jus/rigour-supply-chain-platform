...
public final class FeishuJsapiSignService {
    ...
    public FeishuJsapiSignService(FeishuJsapiClient client, String appId, List<String> allowedOrigins) {
        this(client, appId, allowedOrigins, Clock.systemUTC(), new SecureRandom());
    }
    ...
    public SignResult sign(String url) {
        ...
        if (!allowedOrigins().contains(pageOrigin)) {
        ...
        String signature = sha1("jsapi_ticket=" + ticket
                ...
                + "&timestamp=" + timestamp
                + "&url=" + url);
        log.info("飞书 JSSDK 签名成功 requestId={} origin={} appId={} elapsedMs={}",
                RequestContext.getRequestId(), pageOrigin, appId, elapsedMillis(startedAt));
        return new SignResult(appId, timestamp, nonceStr, signature);
    }
    ...
    public record SignResult(String appId, long timestamp, String nonceStr, String signature) { }
}
