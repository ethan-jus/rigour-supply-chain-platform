package com.rigour.settings.infrastructure.persistence.settings;
import com.rigour.settings.application.port.out.SupplyReadinessStore;
...
public class JdbcSupplyReadinessStore implements SupplyReadinessStore {
 ...
 public SupplyReadinessView inspect(String tenant){
  ...
  String data=jdbc.queryForObject("SELECT CONCAT(COUNT(*),':',COALESCE(SUM(revision),0)) FROM data_dictionary WHERE tenant_key=?",String.class,tenant);
  var checks=new ArrayList<SupplyReadinessView.Check>();
  return new SupplyReadinessView("settings",1,schema+":"+data,checks);
 }
}
