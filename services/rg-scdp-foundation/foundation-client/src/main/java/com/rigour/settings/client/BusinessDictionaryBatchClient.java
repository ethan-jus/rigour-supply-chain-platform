 ...
 * 各领域服务复用的业务字典批处理客户端。
 *
 * <p>调用方只声明明确字段观察值；本类负责按 dictionaryCode 去重并批量解析、按来源值精确解析、 在 Settings
 * 不可用时返回审计告警而不抛出异常中断主数据落库。它不会扫描任意来源字段， 也不会把第三方字段定义成业务字典结构。
 */
public final class BusinessDictionaryBatchClient {
    ...
    public BusinessDictionaryBatchClient(
     ...
     */
    public Audit sync(
            CallerIdentity caller, String sourceType, Collection<Observation> observations) {
        ...
        for (Map.Entry<DictionaryKey, Map<SourceKey, Long>> entry : grouped.entrySet()) {
            ...
            try {
                ...
                entry.getValue()
                        .forEach(
                                (value, count) -> {
                                    DictValueResolution target =
                                            result.resolutions() == null
                                                    ? null
                                                    : result.resolutions().get(value.value());
                                    // 兼容尚未升级的 Settings；升级后以服务端解析结果为准，不在客户端复制业务映射。
                                    if (result.resolutions() == null
                                            && active.containsKey(
                                                    sourceItemCode(
                                                            key.dictionaryCode(), value.value()))) {
                                        target =
                                                new DictValueResolution(
                                                        key.dictionaryCode(),
                                                        sourceItemCode(
                                                                key.dictionaryCode(),
                                                                value.value()));
                                ...
                                (value, count) ->
                                        issues.add(
                                                new MappingIssue(
                                                        ...
                                                        count)));
                log.warn(
                        "业务字典批量解析不可用 tenantId={} sourceType={} dictionaryCode={} errorType={}"
                            + " reason={}",
                        caller.tenantId(),
                        text(sourceType),
                        key.dictionaryCode(),
        ...
        issues.sort(
                Comparator.comparing(MappingIssue::dictionaryCode)
                        .thenComparing(MappingIssue::fieldCode)
                        .thenComparing(MappingIssue::sourceValue));
        long unmapped = issues.stream().mapToLong(MappingIssue::count).sum();
        log.info(
                "业务字典批处理完成 tenantId={} sourceType={} dictionaryCount={} observedCount={}"
                    + " unmappedCount={}",
                caller.tenantId(),
                text(sourceType),
                grouped.size(),
    ...
    public static CallerIdentity serviceCaller(String serviceName, String role, UUID tenantId) {
        ...
        return new CallerIdentity(
                ...
                0,
                Set.of(text(role)),
                Set.of(SYNC_PERMISSION));
    }

    private DictSyncResult syncDictionary(
            CallerIdentity caller, DictionaryKey key, Collection<SourceKey> values) {
        URI uri =
                UriComponentsBuilder.fromUri(baseUri)
                        .path(BusinessDictionaryInternalApi.BASE_PATH)
                        .path("/items/sync")
                        .build()
                        .encode()
                        .toUri();
        Map<String, String> context = signedHeaders("POST", uri, caller);
        DictSyncCommand command =
                new DictSyncCommand(
                        key.dictionaryCode(),
                        values.stream()
                                .map(
                                        value ->
                                                new DictSourceValue(
                                                        value.value(), value.sourceName()))
                                .toList());
        ApiResponse<DictSyncResult> response =
                restClient
                        .post()
                        .uri(uri)
                        ...
                        .exchange(
                                (request, httpResponse) ->
                                        readResponse(httpResponse, DICT_SYNC_RESPONSE, "公共业务字典同步"));
        if (response == null
    ...
    private Map<String, String> activeMappings(DictSyncResult result) {
        ...
        Set<String> active = DictionaryItems.activeCodes(result.effective());
        for (DictItemView item : result.effective().items()) {
            if (!active.contains(item.dictionaryItemCode())) continue;
            String previous =
                    mappings.putIfAbsent(item.dictionaryItemCode(), item.dictionaryItemCode());
            if (previous != null && !previous.equals(item.dictionaryItemCode())) {
                throw new IllegalStateException(
                        "公共业务字典存在重复来源值: " + result.effective().dictionary().dictionaryCode());
            }
        }
        return mappings;
    }
    ...
    private Map<DictionaryKey, Map<SourceKey, Long>> group(List<Observation> source) {
        ...
        for (Observation item : source) {
            DictionaryKey key =
                    new DictionaryKey(required(item.dictionaryCode(), "dictionaryCode"));
            Map<String, SourceAccumulator> values =
        ...
        grouped.forEach(
                (key, values) -> {
                    ...
                    values.forEach(
                            (value, item) ->
                                    counts.put(
                                            new SourceKey(item.fieldCode, value, item.sourceName),
                                            item.count));
    ...
    private Map<String, String> signedHeaders(String method, URI uri, CallerIdentity caller) {
        ...
        TrustedContextSigner.SignedContext signed =
                signer.sign(method, uri.getRawPath(), uri.getRawQuery(), headers);
    ...
    private static void requireCaller(CallerIdentity caller) {
        if (caller == null
                ...
                || (!caller.permissions().contains(SYNC_PERMISSION)
    ...
    public record Observation(
            ...
            String sourceScope) {
        public Observation(
                String dictionaryCode, String fieldCode, String sourceValue, String sourceName) {
    ...
    public record MappingIssue(
            String dictionaryCode, String fieldCode, String sourceValue, long count) {}
    ...
    private static final class SourceAccumulator {
        ...
        private SourceAccumulator(String fieldCode, String sourceName, long count) {
            ...
            this.sourceName =
                    sourceName == null || sourceName.isBlank() ? null : sourceName.strip();
            this.count = count;
        }
