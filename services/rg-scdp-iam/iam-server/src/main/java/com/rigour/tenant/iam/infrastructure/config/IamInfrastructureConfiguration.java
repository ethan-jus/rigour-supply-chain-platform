...
import com.rigour.tenant.iam.infrastructure.bootstrap.ScdpClientBootstrapCommand;
import com.rigour.tenant.iam.infrastructure.bootstrap.ScdpClientBootstrapProperties;
...
import com.rigour.tenant.iam.infrastructure.persistence.auth.JdbcIdentityAccessReader;
...
@EnableConfigurationProperties({
    ...
    ScdpClientBootstrapProperties.class,
...
public final class IamInfrastructureConfiguration {
    ...
    FeishuIdentityStore feishuIdentityStore(
        ...
        return new JdbcFeishuIdentityStore(
    ...
    AccessTokenIssuer feishuAccessTokenIssuer(
        ...
        return new JwtAccessTokenIssuer(
                jwtEncoder, identifierGenerator, serverProperties, tokenProperties);
    }
    ...
    @Bean
    IdentityAccessReader identityAccessReader(
            JdbcTemplate jdbcTemplate,
            com.rigour.tenant.iam.application.port.out.AppSettingsStore supplySettings) {
        return new JdbcIdentityAccessReader(jdbcTemplate, supplySettings);
    }

    @Bean
    IdentityAccessService identityAccessService(IdentityAccessReader reader,
            com.rigour.tenant.iam.application.port.out.AppSettingsStore settings) {
        return new IdentityAccessService(reader, settings);
    }
    ...
    AuthService authService(
        ...
        PasswordAuthenticationPolicy policy =
                new PasswordAuthenticationPolicy(
    ...
    AuthenticationProvider iamPasswordAuthenticationProvider(
        ...
        return new IamPasswordAuthenticationProvider(
                authService, new TransactionTemplate(transactionManager));
    }
    ...
    IamSessionLogoutHandler iamSessionLogoutHandler(
            JdbcTemplate jdbcTemplate, PlatformTransactionManager transactionManager) {
        return new IamSessionLogoutHandler(
                jdbcTemplate, new TransactionTemplate(transactionManager));
    }

    @Bean
    @ConditionalOnProperty(
            prefix = "rigour.iam.bootstrap.scdp-client",
            name = "enabled",
            havingValue = "true")
    ScdpClientBootstrapCommand scdpClientBootstrapCommand(
            RegisteredClientRepository repository,
            IdentifierGenerator identifierGenerator,
            ScdpClientBootstrapProperties properties) {
        return new ScdpClientBootstrapCommand(repository, identifierGenerator, properties);
    }

    @Bean
    @ConditionalOnProperty(
            prefix = "rigour.iam.bootstrap.tenant-admin",
            name = "enabled",
            havingValue = "true")
    TenantAdminBootstrapCommand tenantAdminBootstrapCommand(
            ...
            TenantAdminBootstrapProperties properties) {
    ...
    @Bean
    @ConditionalOnProperty(
            prefix = "rigour.iam.bootstrap.local-signing-key",
            name = "enabled",
            havingValue = "true")
    ...
    @Bean
    @ConditionalOnProperty(
            prefix = "rigour.iam.oidc.signing",
            name = "enabled",
            havingValue = "true")
    ...
    JWKSource<SecurityContext> jwkSource(
            ...
            org.springframework.beans.factory.ObjectProvider<LocalSigningKeyBootstrapCommand>
                    localBootstrap)
            throws Exception {
    ...
    SignedIdTokenDecoder signedIdTokenDecoder(
        ...
        decoder.setJwtValidator(
                new JwtClaimValidator<String>("iss", properties.requireIssuer()::equals));
    ...
    OAuth2TokenGenerator<OAuth2Token> oauth2TokenGenerator(
        ...
        jwtGenerator.setJwtCustomizer(
                new IamJwtCustomizer(claimsResolver, properties.getAccessTokenAudience()));
        return new DelegatingOAuth2TokenGenerator(jwtGenerator, new IamRefreshTokenGenerator());
    }
    ...
    @ConditionalOnProperty(
            ...
            name = "enabled",
            havingValue = "true")
    AuthorizationAttributesCipher authorizationAttributesCipher(
            OidcAuthorizationProperties properties) {
    ...
    OAuth2AuthorizationService oauth2AuthorizationService(
            ...
            IdentifierGenerator identifierGenerator,
            org.springframework.beans.factory.ObjectProvider<SignedIdTokenDecoder>
