...
public final class IamCurrentSessionTokenValidator implements OAuth2TokenValidator<Jwt> {
    ...
    public OAuth2TokenValidatorResult validate(Jwt token) {
        try {
            ...
            if (!"TENANT".equals(scope)) return OAuth2TokenValidatorResult.failure(INVALID);
            Integer count = tenantCount(token, sessionId, principalId, sessionVersion, securityVersion);
