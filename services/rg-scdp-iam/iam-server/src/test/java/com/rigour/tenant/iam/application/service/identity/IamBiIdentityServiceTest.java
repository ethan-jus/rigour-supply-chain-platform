...
class IamBiIdentityServiceTest {
    ...
    private final IdentityAccessService access = mock(IdentityAccessService.class);
    ...
    private final IdentityAccessQuery actor = new IdentityAccessQuery("TENANT", USER, TENANT);
    ...
    @Test void ordinaryUserCannotReadAnotherAccountEvenWhenTheyKnowItsId() {
        ...
        verify(access, never()).currentUser(new IdentityAccessQuery("TENANT", OTHER, TENANT));
    }
    @Test void readingAnotherUserRequiresScdpUserAndRoleReadPermissions() {
        when(access.currentUser(actor)).thenReturn(current(USER, Set.of("TENANT_SUPER_ADMIN"), Set.of("analytics:dashboard:read")));
        assertThatThrownBy(() -> service.read(actor, OTHER)).isInstanceOf(AccessDeniedException.class);
        when(access.currentUser(actor)).thenReturn(current(USER, Set.of("TENANT_SUPER_ADMIN"), Set.of("analytics:dashboard:read", "supply:user:read", "supply:role:read")));
        when(access.currentUser(new IdentityAccessQuery("TENANT", OTHER, TENANT))).thenReturn(current(OTHER, Set.of(), Set.of("analytics:dashboard:read")));
        service.read(actor, OTHER);
        verify(store).read(TENANT, OTHER);
    }
    @Test void targetWithoutBiAccessAndPlatformIdentityAreDenied() {
        when(access.currentUser(actor)).thenReturn(current(USER, Set.of("TENANT_SUPER_ADMIN"), Set.of("analytics:dashboard:read", "supply:user:read", "supply:role:read")));
        when(access.currentUser(new IdentityAccessQuery("TENANT", OTHER, TENANT))).thenReturn(current(OTHER, Set.of(), Set.of()));
        assertThatThrownBy(() -> service.read(actor, OTHER)).isInstanceOf(AccessDeniedException.class);
        assertThatThrownBy(() -> service.read(new IdentityAccessQuery("PLATFORM", USER, null), USER)).isInstanceOf(AccessDeniedException.class);
        verifyNoInteractions(store);
    }
    private static CurrentUser current(UUID id, Set<String> roles, Set<String> permissions) {
        return new CurrentUser(id, TENANT, "tenant", "TENANT", "account", "display", roles, permissions);
    }
}
