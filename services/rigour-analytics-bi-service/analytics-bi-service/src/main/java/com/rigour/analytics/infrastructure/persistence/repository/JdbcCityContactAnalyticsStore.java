...
public class JdbcCityContactAnalyticsStore implements CityContactAnalyticsStore {
    ...
    public JdbcCityContactAnalyticsStore(DataSource dataSource) { jdbc = new NamedParameterJdbcTemplate(dataSource); }
    @Override public List<String> tenantIds() {
        return jdbc.query("SELECT DISTINCT BIN_TO_UUID(tenant_id) AS tenant_id FROM rigour_sales_work.temp_sales_checkin_submission",
                new MapSqlParameterSource(), (rs, index) -> rs.getString("tenant_id"));
    }
    ...
    public SourceRefreshResult refresh(String tenantId, Instant syncedAt) {
        ...
        jdbc.update("""
                ...
                SELECT :tenant, area_code, area_name FROM rigour_crm.crm_customer_area
                 WHERE tenant_id = UUID_TO_BIN(:tenant) AND deleted = 0 AND status = 'ACTIVE'
                   AND NOT EXISTS (SELECT 1 FROM rigour_crm.crm_customer_area child
                        WHERE child.tenant_id = rigour_crm.crm_customer_area.tenant_id
                          AND child.parent_area_code = rigour_crm.crm_customer_area.area_code
        ...
        int count = jdbc.update("""
                ...
                SELECT :tenant, BIN_TO_UUID(s.id), BIN_TO_UUID(s.store_id), NULLIF(TRIM(st.city), ''),
                       a.region_code, s.submitted_at, s.review_status, BIN_TO_UUID(s.salesperson_id),
                       e.employee_code, c.customer_id, c.customer_code
                  FROM rigour_sales_work.temp_sales_checkin_submission s
                  JOIN rigour_sales_work.temp_sales_checkin_store st ON st.tenant_id = s.tenant_id AND st.id = s.store_id
                  LEFT JOIN rigour_sales_work.temp_sales_checkin_employee_link e
                    ON e.tenant_id = s.tenant_id AND e.salesperson_id = s.salesperson_id
                  LEFT JOIN rigour_sales_work.temp_sales_checkin_customer_link c
                  ...
                  LEFT JOIN (SELECT TRIM(city_name) city_name, MIN(region_code) region_code
                        ...
                        GROUP BY TRIM(city_name) HAVING COUNT(*) = 1) a ON a.city_name = TRIM(st.city)
                 WHERE s.tenant_id = UUID_TO_BIN(:tenant) AND s.status = 'SUBMITTED'
                   AND s.deletion_state = 'NONE' AND s.submitted_at IS NOT NULL
                """, args);
        jdbc.update("DELETE FROM bi_sales_contact_snapshot WHERE tenant_id = :tenant", args);
        jdbc.update("INSERT INTO bi_sales_contact_snapshot (tenant_id, synced_time, business_links_ready) VALUES (:tenant, :synced, 1)", args);
        return new SourceRefreshResult("SALES_SUBMITTED_VISIT", "Sales已提交拜访", (long) count, (long) count, 0L, syncedAt);
    }
    @Override @Transactional(readOnly = true)
    public Snapshot read(String tenantId, Instant from, Instant to, String regionCode, String ownerStaffCode) {
        var args = new MapSqlParameterSource("tenant", tenantId).addValue("from", local(from))
                .addValue("to", local(to)).addValue("region", regionCode).addValue("owner", ownerStaffCode);
        var snapshots = jdbc.query("SELECT synced_time, business_links_ready FROM bi_sales_contact_snapshot WHERE tenant_id = :tenant", args,
                (rs, index) -> new Snapshot(rs.getObject("synced_time", LocalDateTime.class).toInstant(ZoneOffset.UTC),
                        rs.getBoolean("business_links_ready"), List.of()));
        ...
        var rows = jdbc.query("""
                ...
                """, args, (rs, index) -> new City(rs.getString("region_code"), rs.getString("city_name"),
                        rs.getLong("contacted"), rs.getLong("approved"), rs.getLong("contacted") - rs.getLong("approved") - rs.getLong("flagged"), rs.getLong("flagged"),
                        snapshot.businessLinksReady() ? rs.getLong("linked") : null,
                        snapshot.businessLinksReady() ? rs.getLong("contacted") - rs.getLong("linked") : null));
