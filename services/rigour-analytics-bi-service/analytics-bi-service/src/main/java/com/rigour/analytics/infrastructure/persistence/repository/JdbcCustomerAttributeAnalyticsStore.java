...
public class JdbcCustomerAttributeAnalyticsStore implements CustomerAttributeAnalyticsStore {
    ...
    public JdbcCustomerAttributeAnalyticsStore(DataSource dataSource) { jdbc = new NamedParameterJdbcTemplate(dataSource); }
    ...
    public Snapshot read(String tenantId, Instant from, Instant to, String regionCode, String employeeCode) {
        ...
        var times = jdbc.query("SELECT synced_time FROM bi_customer_attribute_snapshot WHERE tenant_id = :tenant", args,
        ...
        boolean ordersReady = !jdbc.query("""
                ...
                """, args, (rs, i) -> rs.getString(1)).isEmpty();
        return new Snapshot(times.getFirst(), items(args, false, ordersReady), items(args, true, ordersReady));
    }
    ...
    private List<Item> items(MapSqlParameterSource args, boolean category, boolean ordersReady) {
        ...
        return jdbc.query("""
                ...
                """.formatted(column, column), args, (rs, i) -> {
                    String name = rs.getString("attribute_name");
                    return new Item(name == null ? "未填写" : name, name == null, rs.getLong("customer_count"),
