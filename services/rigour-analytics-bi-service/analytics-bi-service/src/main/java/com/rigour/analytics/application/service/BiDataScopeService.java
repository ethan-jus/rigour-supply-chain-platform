...
public final class BiDataScopeService {
    private static final List<String> RESTRICTED_SUBJECTS = List.of(
            "INVENTORY", "PROCUREMENT", "CITY_COST", "RECONCILIATION", "SOURCE_GOVERNANCE",
            "LEGACY_PERIOD_RECEIPTS");
    private static final List<String> CITY_RESTRICTED_SUBJECTS = List.of(
            "INVENTORY", "PROCUREMENT", "RECONCILIATION", "SOURCE_GOVERNANCE");
    private final BiDataScopeStore store;
    private final Clock clock;
    private final BiDataScopeRenewer renewer;

    public BiDataScopeService(BiDataScopeStore store, Clock analyticsClock, BiDataScopeRenewer renewer) {
    ...
    public BiEffectiveScopeView effective() {
        ...
        if (actor.roles().contains("TENANT_SUPER_ADMIN")) {
            return new BiEffectiveScopeView("TENANT", "AUTHORIZED", null, List.of(), null, null,
                    null, null, true, List.of());
        }
        var identity = store.identity(actor.tenantId().toString(), actor.userId().toString()).orElse(null);
        if (identity == null) return denied("IDENTITY_UNAVAILABLE", "账号尚未关联可验证的 HR 员工和 CRM 销售，请联系管理员");
        if (blank(identity.employeeCode()) || blank(identity.ownerStaffCode())
                || blank(identity.iamBindingRef()) || blank(identity.hrEmployeeRef()) || blank(identity.crmEmployeeRef())) {
            return denied("IDENTITY_INCOMPLETE", "员工与销售身份关联不完整，暂不能读取经营数据");
        }
        if (identity.verifiedAt() == null || identity.expiresAt() == null
        ...
        var grants = sourceGrants.stream()
                .filter(grant -> actor.roles().contains(grant.roleCode())).toList();
        if (grants.isEmpty()) return denied("SCOPE_UNAVAILABLE", "尚未配置可验证的 BI 数据范围");
        if (grants.stream().anyMatch(grant -> blank(grant.iamPolicyRef()) || blank(grant.regionCode())
                || !List.of("SELF", "MY_CITY", "MY_REGION").contains(grant.scopeType()))) {
        ...
        var regions = grants.stream().filter(grant -> !cityScope || !"SELF".equals(grant.scopeType()))
                .map(BiDataScopeStore.Grant::regionCode).distinct().sorted().toList();
        String owner = cityScope ? null : identity.ownerStaffCode();
        return new BiEffectiveScopeView(cityScope ? "CITY" : "SELF", "AUTHORIZED", null,
                regions, identity.employeeCode(), identity.ownerStaffCode(), regions.getFirst(), owner,
                false, cityScope ? CITY_RESTRICTED_SUBJECTS : RESTRICTED_SUBJECTS);
    }
    ...
    public ScopedSelection resolve(String regionCode, String ownerStaffCode) {
        ...
        if (scope.globalGovernance()) return new ScopedSelection(actor.tenantId().toString(), region, owner, true);
        ...
        if ("SELF".equals(scope.accessLevel())) {
            if (owner != null && !owner.equals(scope.ownerStaffCode())) throw deniedException("bi-owner-scope");
    ...
    public void requireObjectScope(String objectRegionCode, String objectOwnerStaffCode) {
        var scope = effective();
        requireAccessible(scope);
        if (scope.globalGovernance()) return;
        if (blank(objectRegionCode) || !scope.regionCodes().contains(objectRegionCode)
                || ("SELF".equals(scope.accessLevel()) && !scope.ownerStaffCode().equals(objectOwnerStaffCode))) {
            throw deniedException("bi-object-scope");
        }
    }
    ...
    public SupplyDashboardFilterOptionsView restrictedFilterOptions() {
        var scope = effective();
        requireAccessible(scope);
        if (scope.globalGovernance()) throw new IllegalStateException("Use tenant filter options for tenant administrators");
        return store.filterOptions(actor().tenantId().toString(), scope.regionCodes(),
    ...
    private static BiEffectiveScopeView denied(String code, String reason) {
        return new BiEffectiveScopeView("DENIED", code, reason, List.of(), null, null,
                null, null, false, RESTRICTED_SUBJECTS);
    }
    ...
    public record ScopedSelection(String tenantId, String regionCode, String ownerStaffCode, boolean fullTenant) { }
}
