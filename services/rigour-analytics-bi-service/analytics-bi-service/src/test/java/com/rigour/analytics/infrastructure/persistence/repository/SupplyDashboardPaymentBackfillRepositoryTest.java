...
public class SupplyDashboardPaymentBackfillRepositoryTest {
    ...
    void setUp() {
        var dataSource = new UnpooledDataSource("org.h2.Driver",
                "jdbc:h2:mem:payment_backfill_" + UUID.randomUUID()
        ...
        jdbc.execute("CREATE SCHEMA rigour_order");
        jdbc.execute("CREATE SCHEMA rigour_crm");
        jdbc.execute("""
                CREATE TABLE rigour_order.order_payment_record (
    ...
    void healthQueryUsesMigratedPaymentIdAndExcludesOtherTenantsAndDeletedRows() {
        ...
        long signature = crc32("101|10|BJ|S1|C1|20|100.250000|BANK")
                + crc32("102|10|BJ|S2|C1|20|25.125000|BANK");
        assertThat(decimal(health, "rowSignature")).isEqualByComparingTo(BigDecimal.valueOf(signature));
    }

    @Test
    void emptyTenantReturnsZeroHealthInsteadOfFailing() {
        Map<String, Object> health = mapper.biSalesPaymentFactBackfillHealth(TENANT);
        for (String key : new String[] {"rowCount", "amount", "regionCount", "ownerCount", "rowSignature"}) {
    ...
    void matchingPaymentProjectionDoesNotTriggerRepeatedBackfill(
        ...
        jdbc.update("INSERT INTO rigour_order.order_sales_order VALUES (?, 10, 20, 'BJ', ?)", TENANT, orderOwner);
        jdbc.update("INSERT INTO rigour_crm.crm_customer VALUES (?, 20, 'SH', ?)", TENANT, customerOwner);
        jdbc.update("INSERT INTO rigour_order.order_payment_record VALUES (?, 101, 10, 20, ?, 'BANK', 125.375, 0)",
                TENANT, collector);
        ...
        jdbc.update("UPDATE bi_sales_payment_fact SET payment_id = 999 WHERE tenant_id = ?", TENANT);
        session.clearCache();
        assertThat(repository.refreshTargetNeedsBackfill(TENANT, "ORDER_PAYMENT_RECORD")).isTrue();
    }

    private void fact(String tenant, long paymentId, String owner, String collector, String amount, int deleted) {
        jdbc.update("""
                ...
                """, tenant, paymentId, owner, collector, new BigDecimal(amount), deleted);
    }

    private static BigDecimal decimal(Map<String, Object> values, String key) {
        Object value = values.entrySet().stream().filter(entry -> entry.getKey().equalsIgnoreCase(key))
