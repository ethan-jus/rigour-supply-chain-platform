...
class BiReconciliationOnlineCaptureRepositoryTest {
    ...
    private final MybatisBiReconciliationReviewRepository repository =
            new MybatisBiReconciliationReviewRepository(mapper, json);

    @Test
    void sourceSelectPermissionFailureHasStableActionableErrorWithoutSqlOrAccountDetails() {
        var denied =
                new SQLException(
                        "SELECT command denied to user 'private_reader' for table"
        ...
        var wrapped =
                new BadSqlGrammarException(
                        ...
                        denied);
        when(mapper.onlineCapture(
                        "tenant",
                        ID,
                        MybatisBiReconciliationReviewRepository.MAX_ONLINE_PAYLOAD_BYTES))
                .thenThrow(wrapped);
        var failure =
                catchThrowableOfType(
                        () -> repository.onlineCapture("tenant", ID), BusinessException.class);
                ...
                .satisfies(
                        detail -> {
                            assertThat(detail.reason())
                                    .isEqualTo("BI_ONLINE_SOURCE_READ_NOT_PROVISIONED");
    ...
    @Test
    void nestedJdbcColumnReadDenialAlsoHasStableProvisioningError() {
        ...
        when(mapper.onlineCapture(
                ...
                .thenThrow(
                        new IllegalStateException(
                                "mapper wrapper",
                                new BadSqlGrammarException("capture", "private SQL", chain)));
        var failure =
                catchThrowableOfType(
                        () -> repository.onlineCapture("tenant", ID), BusinessException.class);
        assertThat(failure.getDetails().getFirst().reason())
                .isEqualTo("BI_ONLINE_SOURCE_READ_NOT_PROVISIONED");
    }

    @Test
    void unrelatedDatabaseFailuresAreNotMisreportedAsMissingSourceSelectPermission() {
        for (SQLException sql :
                List.of(
                        new SQLException("syntax", "42000", 1064),
                        new SQLException("missing table", "42S02", 1146),
                        new SQLException("database unavailable", "08001", 0),
    ...
    @Test
    void legacySourceReadsAreNotRelabeledAsOnlineCapturePermissionFailures() {
        var wrapped =
                new BadSqlGrammarException(
                        "file", "SQL", new SQLException("table command denied", "42000", 1142));
    ...
    @Test
    void captureQueryEnforcesTenantCompletedEvidenceAndBoundedPayloadBeforeTransfer()
            throws Exception {
        String sql =
                String.join(
                        "\n",
                        BiReconciliationReviewMapper.class
                                .getMethod("onlineCapture", String.class, String.class, int.class)
                                .getAnnotation(Select.class)
                                .value());
        assertThat(sql)
                .contains(
                        "bi_source_integration_integration_feishu_online_capture",
                        "tenant_id = UUID_TO_BIN(#{tenant})",
                        "id = UUID_TO_BIN(#{captureId})",
                        "complete = TRUE",
                        "completed_at IS NOT NULL",
                        "OCTET_LENGTH(payload_json) <= #{maxPayloadBytes}")
    ...
    @Test
    void completedCapturePreservesUtcWindowAndSourceIdentityNotOrderIdentity() {
        ...
        assertThat(capture.rows())
                .allSatisfy(
                        r ->
                                assertThat(r)
                                        .containsKey("recordId")
                                        .doesNotContainKey("sourceDocumentNo"));
    }

    @Test
    void filteredDatabaseFlagOrPayloadViewPreventsFullSourceClaim() {
        var row = evidence(payload(null));
    ...
    @Test
    void optionalProductLookupIsIncludedInCountsButCannotReplaceMandatoryOrderTable() {
        String payload =
                json.writeValueAsString(
                        Map.of(
                                "tables",
                                List.of(
                                        table("FEISHU_SALES_ORDER", "orders", null),
                                        table("FEISHU_SALES_ORDER_LINE", "lines", null),
                                        table("FEISHU_PRODUCT", "products", null))));
                        ...
                        json.writeValueAsString(
                                Map.of(
                                        ...
                                        List.of(
                                                table("FEISHU_PRODUCT", "products", null),
                                                table(
                                                        "FEISHU_SALES_ORDER_LINE",
                                                        "lines",
                                                        null))))));
        assertThatThrownBy(() -> repository.onlineCapture("tenant", ID))
                .hasMessageContaining("缺少销售订单");
    }

    @Test
    void incompleteOrUnfinishedSnapshotIsNotReadable() {
        var row = evidence(payload(null));
    ...
    @Test
    void missingDuplicateOrEmptyTablesCannotGenerateMatchingEvidence() {
        for (String invalid :
                List.of(
                        "{\"tables\":[]}",
                        json.writeValueAsString(
                                Map.of(
                                        "tables",
                                        List.of(table("FEISHU_SALES_ORDER", "orders", null)))),
                        json.writeValueAsString(
                                Map.of(
                                        "tables",
                                        List.of(
                                                table("FEISHU_SALES_ORDER", "orders", null),
                                                table("FEISHU_SALES_ORDER", "orders2", null)))))) {
                        ...
                        json.writeValueAsString(
                                Map.of(
                                        ...
                                        List.of(
                                                ...
                                                empty)))));
        assertThatThrownBy(() -> repository.onlineCapture("tenant", ID))
                .hasMessageContaining("没有可核对记录");
    }

    @Test
    void missingFieldsRecordIdentityOrPartialTableIsRejectedEvenWithCompleteFlag() {
        for (Map<String, Object> invalid :
                List.of(
                        Map.<String, Object>of("recordId", "recOne"),
                        Map.<String, Object>of("fields", Map.of("订单号", "DD1")),
                        Map.<String, Object>of("recordId", "recOne", "fields", Map.of()))) {
                            ...
                            json.writeValueAsString(
                                    Map.of(
                                            ...
                                            List.of(
                                                    ...
                                                    table)))));
            assertThatThrownBy(() -> repository.onlineCapture("tenant", ID))
                    .hasMessageContaining("记录标识或业务字段");
        }
        var table = table("FEISHU_SALES_ORDER_LINE", "lines", null);
                        ...
                        json.writeValueAsString(
                                Map.of(
                                        ...
                                        List.of(
                                                ...
                                                table)))));
        assertThatThrownBy(() -> repository.onlineCapture("tenant", ID))
                .hasMessageContaining("未完整采集");
    }

    @Test
    void duplicateRecordsMismatchedCountsInvalidWindowsAndOversizeEvidenceFailClosed() {
        var row = evidence(payload(null));
        ...
        row.put("recordCount", 3);
        assertThatThrownBy(() -> repository.onlineCapture("tenant", ID))
                .hasMessageContaining("数量与采集证据不一致");
        row.put("recordCount", 2);
        row.put(
                "payloadBytes",
                MybatisBiReconciliationReviewRepository.MAX_ONLINE_PAYLOAD_BYTES + 1L);
        row.remove("payloadJson");
        assertThatThrownBy(() -> repository.onlineCapture("tenant", ID))
                .hasMessageContaining("32MiB");
    ...
    @Test
    void payloadChecksumAndMalformedJsonAreValidated() {
        var row = evidence(payload(null));
    ...
    @Test
    void oldSavedJsonAndOldConstructorsRemainReadableWithoutOnlineEvidence() {
        String legacy =
                """
...
""";
        ...
        var command =
                json.readValue(
                        "{\"batchId\":\"batch\",\"sourceDeclaredComplete\":true}",
                        BiReconciliationReview.Command.class);
        assertThat(command.onlineCaptureId()).isNull();
        assertThat(
                        new BiReconciliationReview.Command("batch", null, null, null, null, true)
                                .onlineCaptureId())
                .isNull();
    }

    @Test
    void onlineReviewRoundTripStoresCaptureIdentityAndNeverRequeriesSourceOnGet() {
        stub(evidence(payload(null)));
        var version = repository.onlineCapture("tenant", ID).orElseThrow().version();
        var now = Instant.parse("2026-09-14T02:00:00Z");
        var review =
                new BiReconciliationReview(
                        ID,
                        now,
                        now,
                        now,
        ...
        verify(mapper)
                .insert(
                        ...
                        any(),
                        any(),
                        eq(json.writeValueAsString(review)));
        clearInvocations(mapper);
    ...
    private String payload(String view) {
        return json.writeValueAsString(
                Map.of(
                        "tables",
                        List.of(
                                table("FEISHU_SALES_ORDER", "orders", view),
                                table("FEISHU_SALES_ORDER_LINE", "lines", null))));
    }
    ...
    private static Map<String, Object> table(String code, String id, String view) {
        ...
        table.put("tableCode", code);
        table.put("viewId", view);
    ...
    private static Map<String, Object> evidence(String payload) {
        ...
        row.put("captureId", ID);
        row.put("sourceId", "sales-source");
        row.put("sourceName", "销售订单");
        row.put("sourceUrl", "https://example.feishu.cn/base/source");
        row.put("startedAt", LocalDateTime.of(2026, 9, 14, 1, 0));
        row.put("completedAt", LocalDateTime.of(2026, 9, 14, 1, 1));
        row.put("complete", true);
        row.put("filtered", false);
        row.put("recordCount", 2);
        row.put("pageCount", 2);
        row.put("payloadJson", payload);
        row.put("payloadBytes", payload.getBytes(StandardCharsets.UTF_8).length);
        try {
            row.put(
                    "checksum",
                    HexFormat.of()
