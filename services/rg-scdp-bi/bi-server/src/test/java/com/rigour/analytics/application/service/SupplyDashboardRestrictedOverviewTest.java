...
class SupplyDashboardRestrictedOverviewTest {
    ...
    private final SupplyDashboardQueryService service =
    ...
    void setup() throws Exception {
        ...
        when(scopes.effective())
                .thenReturn(
                        new com.rigour.analytics.api.v1.model.BiEffectiveScopeView(
                                "CITY",
                                "AUTHORIZED",
                                null,
                                List.of("BJ"),
                                ...
                                false,
                                List.of("INVENTORY")));
        when(scopes.resolve(any(), any()))
                .thenAnswer(
                        call ->
                                new BiDataScopeService.ScopedSelection(
                                        tenant.toString(), "BJ", call.getArgument(1), false));
    }
    ...
    @Test
    void wholeCityRetainsItsRealCostAndScopedCityAndSalesTargets() {
        var result = service.overview(NOW, NOW, "BJ", null, null, null, null);
        assertThat(result.cityCostTrend()).hasSize(1);
        assertThat(result.cityCostRanking())
                .singleElement()
                .satisfies(cost -> assertThat(cost.regionCode()).isEqualTo("BJ"));
        ...
        assertThat(result.salesTargetCompletions())
                .singleElement()
                .satisfies(target -> assertThat(target.dimensionCode()).isEqualTo("E1"));
        assertThat(result.collectionTrend()).hasSize(1);
    }

    @Test
    void selfOrGranularFilterCannotExposeCitywideCostOrCityTargets() {
        for (String[] selection :
                List.of(
                        new String[] {"E1", null, null},
                        new String[] {null, "STORE", null},
                        new String[] {null, null, "FEISHU"})) {
            var result =
                    service.overview(
                            NOW, NOW, "BJ", selection[0], selection[1], null, selection[2]);
        ...
        assertThat(service.overview(NOW, NOW, "BJ", null, null, 1L, null).cityCostRanking())
                .isEmpty();
    }

    @Test
    void selfCanUseScopedCustomerHistoryAndItsCompleteTarget() {
    ...
    @Test
    void
            unsupportedGranularComparisonsDoNotPretendToHaveCategoryCustomerHistoryOrProratedTargets() {
        ...
        assertThat(
                        service.overview(NOW, NOW, "BJ", "E1", "STORE", null, null)
                                .salesTargetCompletions())
                .isEmpty();
        assertThat(
                        service.overview(NOW, NOW, "BJ", "E1", null, null, "FEISHU")
                                .salesTargetCompletions())
                .isEmpty();
    }
    ...
    private static SupplyDashboardData data() throws Exception {
        ...
        var values =
                Map.<String, Object>of(
                        "cityCost",
                        new CityCostSummary(1L, AMOUNT, AMOUNT, NOW),
                        "cityCostTrend",
                        List.of(new TrendPoint("city_cost_amount", "2026-09-12", AMOUNT, AMOUNT)),
                        "collectionTrend",
                        List.of(new TrendPoint("receipt_amount", "2026-09-12", AMOUNT, AMOUNT)),
                        "cityCostRanking",
                        List.of(
                                new CityCostItem(
                                        "BJ",
        ...
        return SupplyDashboardData.class
                .getDeclaredConstructor(
                        Arrays.stream(fields)
                                .map(java.lang.reflect.RecordComponent::getType)
                                .toArray(Class<?>[]::new))
                .newInstance(
                        Arrays.stream(fields).map(field -> values.get(field.getName())).toArray());
    }
}
