...
public final class SupplyDashboardRefreshService {
    ...
    private static final SourceMeta INVENTORY_OPERATION =
            new SourceMeta("ERP_INVENTORY_OPERATION", "采购/发货流转");
    private static final SourceMeta RECONCILIATION =
            new SourceMeta("BI_RECONCILIATION_CURRENT", "对账快照");
    private static final List<SourceMeta> SOURCE_METAS =
            List.of(
                    CUSTOMER,
                    ORDER,
                    ORDER_LINE,
    ...
    public SupplyDashboardRefreshService(
            ...
            @Value("${rigour.analytics.supply-dashboard.refresh.enabled:true}")
                    boolean scheduledEnabled,
            @Value("${rigour.analytics.supply-dashboard.refresh.lookback:PT2H}") Duration lookback,
            @Value("${rigour.analytics.supply-dashboard.refresh.lock-ttl:PT55M}")
                    Duration lockTtl) {
    ...
    private RefreshRun refreshTenant(String tenantId, String jobCode, RefreshSelection selection) {
        ...
        boolean locked =
                store.acquireRefreshLock(tenantId, LOCK_CODE, startedAt, startedAt.plus(lockTtl));
        if (!locked) {
        ...
        try {
            store.synchronizeSourceSnapshots(tenantId);
                ...
                collectRefreshResult(
                        ...
                        () ->
                                refreshIncremental(
                                        ...
                                        (from, to, syncedAt) ->
                                                store.refreshCustomerDim(
                                                        tenantId, from, to, syncedAt)));
            }
                ...
                collectRefreshResult(
                        ...
                        () ->
                                refreshIncremental(
                                        ...
                                        selection.fullRefresh(),
                                        (from, to, syncedAt) ->
                                                store.refreshSalesOrderLineFact(
                                                        tenantId, from, to, syncedAt)));
            }
                ...
                collectRefreshResult(
                        ...
                        () ->
                                refreshIncremental(
                                        ...
                                        selection.fullRefresh(),
                                        (from, to, syncedAt) ->
                                                store.refreshSalesPaymentFact(
                                                        tenantId, from, to, syncedAt)));
            }
            if (selection.refreshesCustomerAttribution()) {
                try {
                    upserted += store.alignFrozenOrderAttribution(tenantId, Instant.now(clock));
                } catch (RuntimeException exception) {
                    failures.add(failureReason("订单冻结归属对齐", exception));
                    log.warn(
                            "供应链 BI 来源刷新失败 tenantId={} sourceCode={} sourceName={} reason={}",
                            tenantId,
                            "FROZEN_ORDER_ATTRIBUTION",
                            "订单冻结归属对齐",
                ...
                collectRefreshResult(
                        ...
                        () ->
                                refreshSnapshot(
                                        ...
                                        INVENTORY,
                                        syncedAt ->
                                                store.refreshInventoryBalanceCurrent(
                                                        tenantId, syncedAt)));
            }
                ...
                collectRefreshResult(
                        ...
                        () ->
                                refreshSnapshot(
                                        ...
                                        INVENTORY_OPERATION,
                                        syncedAt ->
                                                store.refreshInventoryOperationFact(
                                                        tenantId, syncedAt)));
            }
                ...
                collectRefreshResult(
                        ...
                        () ->
                                refreshSnapshot(
                                        ...
                                        RECONCILIATION,
                                        syncedAt ->
                                                store.refreshReconciliationCurrent(
                                                        tenantId,
                                                        monthStart(upperBound),
                                                        upperBound,
                                                        syncedAt)));
            }
            ...
            if (!failures.isEmpty()) {
                ...
                RefreshRun failed =
                        store.failRefreshRun(
                                run.id(), completedAt, pulled, upserted, skipped, reason);
                store.releaseRefreshLock(tenantId, LOCK_CODE, "FAILED", completedAt, reason);
                return failed;
            }
            RefreshRun completed =
            ...
            RefreshRun failed =
                    store.failRefreshRun(run.id(), completedAt, pulled, upserted, skipped, reason);
            store.releaseRefreshLock(tenantId, LOCK_CODE, "FAILED", completedAt, reason);
            return failed;
        }
    }
    ...
    private SourceRefreshResult refreshIncremental(
        ...
        boolean targetNeedsBackfill =
                !fullRefresh && store.refreshTargetNeedsBackfill(tenantId, source.code());
        if (targetNeedsBackfill) {
            log.info("供应链 BI 目标表缺口触发全量补齐 tenantId={} sourceCode={}", tenantId, source.code());
        }
        Instant from =
                fullRefresh || targetNeedsBackfill
                        ? Instant.EPOCH
        ...
        Instant nextWatermark =
                result.watermarkTime() == null ? upperBound : result.watermarkTime();
        store.updateCheckpoint(
                tenantId, source.code(), source.name(), nextWatermark, syncedAt, runId);
        return result;
    }

    private void collectRefreshResult(
