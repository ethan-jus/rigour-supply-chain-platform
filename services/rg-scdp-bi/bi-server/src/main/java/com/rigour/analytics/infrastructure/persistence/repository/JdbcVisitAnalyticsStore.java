...
public class JdbcVisitAnalyticsStore implements VisitAnalyticsStore {
    ...
    public JdbcVisitAnalyticsStore(DataSource dataSource) {
        jdbc = new NamedParameterJdbcTemplate(dataSource);
    }

    @Override
    @Transactional(readOnly = true)
    public VisitAnalyticsView read(
        ...
        var times =
                jdbc.query(
                        "SELECT synced_time FROM bi_sales_contact_snapshot WHERE tenant_id=:tenant"
                            + " AND business_links_ready=1",
                        args,
                        (rs, index) ->
                                rs.getObject("synced_time", LocalDateTime.class)
                                        .toInstant(ZoneOffset.UTC));
        if (times.isEmpty())
            return new VisitAnalyticsView(
                    "NOT_READY", null, from, to, null, List.of(), List.of(), List.of());
        var summary =
                com.rigour.analytics.infrastructure.persistence.scope.BiScopedQueries
                        .queryForObject(
                                jdbc,
                                SCOPE
                                        + """
...
""",
                                ...
                                (rs, index) ->
                                        new Day(
                                                ...
                                                rs.getLong("stores")))
                        .stream()
                        .collect(Collectors.toMap(Day::date, Function.identity()));
        var days = new ArrayList<Day>();
        for (var day = from.atZone(BiBusinessTime.ZONE).toLocalDate();
                !day.isAfter(to.atZone(BiBusinessTime.ZONE).toLocalDate());
                day = day.plusDays(1)) {
