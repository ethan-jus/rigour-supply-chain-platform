...
SELECT BIN_TO_UUID(id) AS batchId, original_file_name AS fileName, file_sha256 AS checksum,
  ...
  FROM bi_source_integration_integration_feishu_import_batch
...
SELECT table_code AS tableCode, source_document_no AS sourceDocumentNo,
  ...
  FROM bi_source_integration_integration_feishu_import_raw_row
...
SELECT o.source_order_no AS orderNo, 'ORDER' AS kind, o.source_order_no AS identityKey,
  ...
  LEFT JOIN bi_source_crm_crm_customer c ON c.tenant_id=o.tenant_id AND c.id=o.customer_id
  LEFT JOIN bi_source_crm_crm_customer_area ca ON ca.tenant_id=UUID_TO_BIN(o.tenant_id)
...
""")
    List<Map<String, Object>> businessOrders(
            @Param("tenant") String tenant, @Param("limit") int limit);
...
SELECT o.source_order_no AS orderNo, 'SKU' AS kind, l.id AS systemLineId,
  ...
  JOIN bi_source_order_order_sales_order o ON o.tenant_id=l.tenant_id AND o.id=l.order_id
  LEFT JOIN bi_source_crm_crm_customer c ON c.tenant_id=o.tenant_id AND c.id=o.customer_id
  LEFT JOIN bi_source_crm_crm_customer_area ca ON ca.tenant_id=UUID_TO_BIN(o.tenant_id)
   AND ca.area_code=COALESCE(NULLIF(o.region_code,''),NULLIF(c.region_code,''))
...
""")
    List<Map<String, Object>> businessLines(
            @Param("tenant") String tenant, @Param("limit") int limit);
...
""")
    ...
    @Select(
            ...
            """)
    String find(
            @Param("tenant") String tenant, @Param("actor") String actor, @Param("id") String id);
