...
public class OperatingWorkspaceService {
    ...
    private static final Map<String, Set<String>> NEXT =
            Map.of(
                    "OPEN",
                    Set.of("OPEN", "IN_PROGRESS", "DISMISSED"),
                    "IN_PROGRESS",
                    Set.of("IN_PROGRESS", "RESOLVED", "DISMISSED"),
                    "RESOLVED",
                    Set.of("OPEN"),
                    "DISMISSED",
                    Set.of("OPEN"));
    ...
    public OperatingWorkspaceService(
            OperatingWorkspaceStore store, Clock analyticsClock, BiDataScopeService scope) {
        this.store = store;
    ...
    public ActionPage actions(
        ...
        return store.actions(
                actor.tenantId().toString(),
                new ActionFilter(
                        optionalChoice(kind, KINDS, "跟进类型"),
                        text(ref, 160, false, "业务对象"),
                        text(selected.regionCode(), 64, false, "城市"),
                        text(selected.ownerStaffCode(), 64, false, "员工"),
                        text(assignee, 160, false, "负责人"),
                        optionalChoice(status, STATUSES, "状态"),
                        page,
                        size,
                        selected.fullTenant()));
    }
    ...
    public ActionView createAction(ActionCommand input) {
        ...
        var subject =
                requireSubject(
                        actor.tenantId().toString(),
                        kind,
                        text(input.businessRef(), 160, true, "业务对象"));
        var command =
                new ActionCommand(
                        kind,
                        subject.businessRef(),
                        subject.businessLabel(),
                        subject.cityCode(),
                        subject.employeeCode(),
                        text(input.assignee(), 160, true, "负责人"),
                        due(input.dueAt()),
                        text(input.note(), 2000, true, "跟进内容"));
        scope.requireObjectScope(command.cityCode(), command.employeeCode());
        scope.requireObjectActionScope(command.cityCode(), command.employeeCode(), ACTION_WRITE);
        if ("STOCK".equals(command.kind())) scope.requireGlobalGovernance();
        requireTargetScope(
                actor.tenantId().toString(), "SALES_OWNER", command.assignee(), ACTION_WRITE);
    ...
    public ActionView updateAction(String id, ActionUpdateCommand input) {
        ...
        var previous =
                store.action(tenant, text(id, 64, true, "跟进"))
                        .orElseThrow(OperatingWorkspaceService::notFound);
        requireActionScope(tenant, previous);
        scope.requireObjectActionScope(previous.cityCode(), previous.employeeCode(), ACTION_WRITE);
        var subject = store.subjects(tenant, previous.kind(), previous.businessRef()).getFirst();
        scope.requireObjectActionScope(subject.cityCode(), subject.employeeCode(), ACTION_WRITE);
        ...
        var command =
                new ActionUpdateCommand(
                        text(input.assignee(), 160, true, "负责人"),
                        due(input.dueAt()),
                        status,
                        text(input.note(), 2000, true, "处理记录"),
                        input.expectedRevision());
        requireTargetScope(tenant, "SALES_OWNER", command.assignee(), ACTION_WRITE);
        return store.updateAction(
                        tenant, actor.userId().toString(), previous, command, Instant.now(clock))
    ...
    private OperatingWorkspaceStore.BusinessSubject requireSubject(
            String tenant, String kind, String ref) {
    ...
    private void requireActionScope(String tenant, ActionView action) {
        ...
        String owner =
                "SELF".equals(access.accessLevel())
                                && action.assignee().equals(access.ownerStaffCode())
                        ? action.assignee()
                        : action.employeeCode();
        ...
        scope.requireObjectScope(
                actual.getFirst().cityCode(),
                "SELF".equals(access.accessLevel())
                                && action.assignee().equals(access.ownerStaffCode())
                        ? action.assignee()
    ...
    private void requireTargetScope(String tenant, String type, String code, String action) {
        ...
        for (String region : regions) {
            scope.requireObjectScope(region, "SALES_OWNER".equals(type) ? code : null);
            if (!READ.equals(action))
                scope.requireObjectActionScope(
                        region, "SALES_OWNER".equals(type) ? code : null, action);
        }
    }
    ...
    private static Instant due(Instant value) {
        if (value == null
                ...
                || !value.isBefore(Instant.parse("2101-01-01T00:00:00Z")))
            throw invalid("请提供有效跟进期限");
        return value;
    }
    ...
    private static BusinessException notFound() {
        return new BusinessException(ErrorCode.NOT_FOUND, "记录不存在或不可访问", List.of());
    }
