...
public class MybatisPlusSupplyDashboardRepository
    ...
    private org.springframework.jdbc.core.JdbcTemplate scopeJdbc;

    /** 导入操作既核验新地区，也锁定并核验既有记录，避免把范围外成本搬入本地区后覆盖。 */
    private void requireCityCostScope(String tenant, List<CityCostImportRow> rows) {
        var context =
        ...
        if (!policy.functionAllowed())
            throw new com.rigour.shared.context.AuthorizationDeniedException(
                    "analytics:city-cost:write");
        var predicate =
        ...
        for (var row : rows) {
            ...
            regions.addAll(
                    scopeJdbc.queryForList(
                            "SELECT region_code FROM bi_city_cost_record WHERE tenant_id=? AND"
                                    + " source_system_code=? AND source_record_id=? FOR UPDATE",
                            String.class,
                            tenant,
                            row.sourceSystemCode(),
                            row.sourceRecordId()));
            for (String region : regions) {
                var args = new java.util.ArrayList<Object>(List.of(region, tenant));
                args.addAll(predicate.args());
                int count =
                        scopeJdbc.queryForObject(
                                "SELECT COUNT(*) FROM (SELECT ? AS region_code) f LEFT JOIN"
                                        + " bi_region_authority a ON a.tenant_id=? AND"
                                        + " a.region_code=f.region_code WHERE "
                                        + predicate.text(),
                                Integer.class,
                                args.toArray());
                if (count != 1)
    ...
    public SupplyDashboardData overview(String tenantId, SupplyDashboardFilter filter) {
        ...
        CityCostSummary cityCost =
                cityCostSummary(mapper.cityCostSummary(tenantId, from, to, filter.regionCode()));
        List<RankingItem> citySalesRanking =
                mapper
                        .citySalesRanking(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::ranking)
                        .toList();
        return new SupplyDashboardData(
                ...
                mapper
                        .salesTrend(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::trend)
                        .toList(),
                mapper
                        .collectionTrend(
                                tenantId,
                        ...
                        .map(MybatisPlusSupplyDashboardRepository::trend)
                        .toList(),
                mapper.cityCostTrend(tenantId, from, to, filter.regionCode()).stream()
                        .map(MybatisPlusSupplyDashboardRepository::trend)
                        .toList(),
                citySalesRanking,
                ...
                mapper
                        .salesMonthlyPerformance(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::salesMonthlyPerformance)
                        .toList(),
                mapper
                ...
                mapper
                        .productSalesRanking(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::productSales)
                        .toList(),
                mapper
                        .skuSalesRanking(
                                tenantId,
                ...
                mapper
                        .customerSegmentSummary(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::customerSegment)
                        .toList(),
                mapper
                        .customerActivityRanking(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::customerActivity)
                        .toList(),
                mapper
                        ...
                        .map(MybatisPlusSupplyDashboardRepository::customerActivity)
                        .toList(),
                mapper.inventoryItemSummary(tenantId, from, to, filter.productCategoryId()).stream()
                        .map(MybatisPlusSupplyDashboardRepository::inventoryItemSummary)
                        .toList(),
                mapper
                        .inventoryReplenishment(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::inventoryReplenishment)
                        .toList(),
                mapper
                        .cityCostRanking(
                                tenantId,
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::cityCost)
                        .toList(),
                mapper
                        .inventoryRisks(tenantId, filter.regionCode(), filter.productCategoryId())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::risk)
                        .toList(),
                mapper.dataFreshness(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::freshness)
                        .toList());
    }
    ...
    public OperatingAnalysisData operatingAnalysis(
        ...
        return new OperatingAnalysisData(
                mapper
                        .salesRanking(
                                tenantId,
                                local(previousFilter.from()),
                                local(previousFilter.to()),
                                previousFilter.regionCode(),
                                previousFilter.ownerStaffCode(),
                                previousFilter.customerTypeCode(),
                                previousFilter.sourceSystemCode())
                        .stream()
                        .map(MybatisPlusSupplyDashboardRepository::ranking)
                        .toList(),
                mapper
    ...
    public TrustData trust(String tenantId) {
        ...
        return new TrustData(
                ...
                mapper.trustSources(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::trustSource)
                        .toList());
    }
    ...
    public ReconciliationData reconciliation(String tenantId, SupplyDashboardFilter filter) {
        ...
        List<Map<String, Object>> rows =
                List.of(
                        mapper.salesOrderReconciliation(
                                tenantId,
                                from,
                                to,
                                filter.regionCode(),
                                ...
                                filter.customerTypeCode(),
                                filter.sourceSystemCode()),
                        mapper.cityAttributionReconciliation(
                                tenantId,
                                from,
                                to,
                                filter.regionCode(),
                        ...
                        mapper.paymentReconciliation(
                                ...
                                filter.customerTypeCode(),
                                filter.sourceSystemCode()),
                        mapper.customerReconciliation(
                                tenantId,
                                filter.regionCode(),
                                filter.ownerStaffCode(),
                                filter.customerTypeCode()),
                        mapper.customerContactReconciliation(
                                tenantId,
                                filter.regionCode(),
                                filter.ownerStaffCode(),
                                filter.customerTypeCode()),
                        ...
                        mapper.orderLineReconciliation(
                                tenantId,
                                from,
                                to,
                                filter.regionCode(),
                                ...
                                filter.productCategoryId(),
                                filter.sourceSystemCode()),
                        mapper.estimatedGrossProfitReconciliation(
                                tenantId,
                                from,
                                to,
                                filter.regionCode(),
                                ...
                                filter.productCategoryId(),
                                filter.sourceSystemCode()),
                        mapper.paymentRiskReconciliation(
                                tenantId,
                                from,
                                to,
                                filter.regionCode(),
                                ...
                                filter.customerTypeCode(),
                                filter.sourceSystemCode()),
                        mapper.inventoryReconciliation(
    ...
    public FilterOptions filterOptions(String tenantId) {
        ...
        return new FilterOptions(
                mapper.regionOptions(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::option)
                        .toList(),
                mapper.salesOwnerOptions(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::option)
                        .toList(),
                mapper.customerTypeOptions(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::option)
                        .toList(),
                mapper.productCategoryOptions(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::option)
                        .toList(),
                mapper.sourceSystemOptions(tenantId).stream()
                        .map(MybatisPlusSupplyDashboardRepository::option)
                        .toList());
    }

    @org.springframework.beans.factory.annotation.Autowired
    private com.rigour.analytics.infrastructure.persistence.scope.BiSourceSnapshotProjector sources;

    @org.springframework.beans.factory.annotation.Autowired
    private com.rigour.analytics.infrastructure.persistence.scope.BiAuthorityProjector authority;
    ...
    public boolean acquireRefreshLock(
            String tenantId, String lockCode, Instant now, Instant lockedUntil) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
    ...
    public void releaseRefreshLock(
        ...
        getBaseMapper()
                .releaseRefreshLock(tenantId, lockCode, statusCode, local(now), failureReason);
    }
    ...
    public RefreshRun failRefreshRun(
            ...
            String failureReason) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        mapper.failRefreshRun(
                runId, local(completedAt), pulledCount, upsertedCount, skippedCount, failureReason);
        return run(mapper.refreshRun(runId));
    }
    ...
    public Optional<Instant> checkpointWatermark(String tenantId, String sourceCode) {
        return Optional.ofNullable(
                instant(
                        getBaseMapper().checkpointWatermark(tenantId, sourceCode),
                        "watermarkTime"));
    }
    ...
    public boolean refreshTargetNeedsBackfill(String tenantId, String sourceCode) {
        ...
        return switch (sourceCode) {
            case "CRM_CUSTOMER" ->
                    targetBehind(
                            mapper.crmCustomerSourceTotal(tenantId),
                            mapper.biCustomerDimTotal(tenantId));
            case "ORDER_SALES_ORDER" ->
                    targetBehind(
                            mapper.orderSourceBackfillHealth(tenantId),
                            mapper.biSalesOrderFactBackfillHealth(tenantId));
            case "ORDER_SALES_ORDER_LINE" ->
                    targetBehind(
                            mapper.orderLineSourceBackfillHealth(tenantId),
                            mapper.biSalesOrderLineFactBackfillHealth(tenantId));
            case "ERP_PRODUCT" ->
                    targetBehind(
                            mapper.productSourceTotal(tenantId),
                            mapper.biProductDimTotal(tenantId));
            case "ORDER_PAYMENT_RECORD" ->
                    targetBehind(
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SourceRefreshResult refreshCustomerDim(
        ...
        int affected =
                mapper.upsertCustomerDimFromSource(tenantId, localFrom, localTo, localSyncedAt);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SourceRefreshResult refreshSalesOrderFact(
            String tenantId, Instant from, Instant to, Instant syncedAt) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        LocalDateTime localFrom = local(from);
        LocalDateTime localTo = local(to);
        ...
        int affected =
                mapper.upsertSalesOrderFactFromSource(
                        tenantId, localFrom, localTo, local(syncedAt));
        return sourceResult("ORDER_SALES_ORDER", "销售订单", summary, affected);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public SourceRefreshResult refreshSalesOrderLineFact(
            String tenantId, Instant from, Instant to, Instant syncedAt) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        LocalDateTime localFrom = local(from);
        LocalDateTime localTo = local(to);
        ...
        int affected =
                mapper.upsertSalesOrderLineFactFromSource(
                        tenantId, localFrom, localTo, local(syncedAt));
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SourceRefreshResult refreshProductDim(
            String tenantId, Instant from, Instant to, Instant syncedAt) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        LocalDateTime localFrom = local(from);
        LocalDateTime localTo = local(to);
        ...
        int affected =
                mapper.upsertProductDimFromSource(tenantId, localFrom, localTo, local(syncedAt));
        mapper.retireCategorySnapshot(tenantId);
        affected += mapper.refreshCategorySnapshot(tenantId, local(syncedAt));
        var closure =
                com.rigour.analytics.application.model.ProductCategoryHierarchy.closure(
                        mapper.categoryHierarchy(tenantId));
        mapper.clearCategoryClosure(tenantId);
        for (int index = 0; index < closure.size(); index += 500)
            affected +=
                    mapper.insertCategoryClosure(
                            tenantId,
                            closure.subList(index, Math.min(index + 500, closure.size())));
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SourceRefreshResult refreshSalesPaymentFact(
            String tenantId, Instant from, Instant to, Instant syncedAt) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        LocalDateTime localFrom = local(from);
        LocalDateTime localTo = local(to);
        ...
        int affected =
                mapper.upsertSalesPaymentFactFromSource(
                        tenantId, localFrom, localTo, local(syncedAt));
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public long alignFrozenOrderAttribution(String tenantId, Instant syncedAt) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        LocalDateTime localSyncedAt = local(syncedAt);
        long affected = 0;
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SourceRefreshResult refreshReconciliationCurrent(
            String tenantId, Instant from, Instant to, Instant observedAt) {
        SupplyDashboardQueryMapper mapper = getBaseMapper();
        LocalDateTime localFrom = local(from);
        LocalDateTime localTo = local(to);
        ...
        List<ReconciliationItem> items =
                List.of(
                        reconciliation(
                                mapper.salesOrderReconciliation(
                                        tenantId, localFrom, localTo, null, null, null, null)),
                        reconciliation(
                                mapper.cityAttributionReconciliation(
                                        tenantId, localFrom, localTo, null, null, null, null)),
                        reconciliation(
                        ...
                        reconciliation(
                                mapper.customerContactReconciliation(tenantId, null, null, null)),
        ...
        for (ReconciliationItem item : items) {
            affected +=
                    mapper.upsertReconciliationCurrent(
                            tenantId,
                            item.subjectCode(),
                            item.subjectName(),
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public CityCostImportResult importCityCostRecords(
            String tenantId, List<CityCostImportRow> rows, Instant importedAt) {
        requireCityCostScope(tenantId, rows);
        int upserted = 0;
        LocalDateTime localImportedAt = local(importedAt);
        for (CityCostImportRow row : rows) {
            int affected =
                    getBaseMapper()
                            .upsertCityCostRecord(
                                    tenantId,
                                    row.sourceSystemCode(),
                                    row.sourceRecordId(),
                                    row.regionCode(),
        ...
        var cities =
                mapper
                        .salesRankingCityAttributions(
                                ...
                                filter.sourceSystemCode())
                        .stream()
                        .collect(Collectors.groupingBy(row -> text(row, "dimensionCode")));
        return ranks.stream()
                .map(
                        rank -> {
                            ...
                            var currentCity =
                                    attribution.stream()
                                            .map(row -> text(row, "currentRegionName"))
                                            .filter(Objects::nonNull)
                                            .findFirst()
                                            .orElse(null);
                            var orderCities =
                                    attribution.stream()
                                            .map(row -> text(row, "orderRegionName"))
                                            .filter(Objects::nonNull)
                                            .distinct()
                                            .sorted()
                                            .toList();
    ...
    private static boolean amountDiffers(
