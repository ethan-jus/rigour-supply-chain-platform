...
(async()=>{
 ...
 try{
  ...
  await context.route('**/*',async route=>{
   ...
   if(url.pathname===api+'/submissions/attendance-summary')return json({totalVisits:1,checkedInSalespeople:1,pendingReviewTotal:1,items:[{date:'2026-09-07',city:'杭州',salespersonId:id(70),salespersonName:'虚构销售',visitCount:1,storeCount:1,audioCount:0,firstCheckinAt:item.completedAt,lastCheckinAt:item.completedAt,pendingReviewCount:1}],page:0,size:50,totalElements:1,totalPages:1});
