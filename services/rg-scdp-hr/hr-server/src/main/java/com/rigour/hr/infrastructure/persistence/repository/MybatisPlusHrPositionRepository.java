...
/** MyBatis-Plus HR 岗位仓储；保持与 ERP 主数据查询风格一致。 */
@Repository
public class MybatisPlusHrPositionRepository
    ...
    private final JdbcHrOrganizationStore organization;
    private final org.springframework.jdbc.core.JdbcTemplate jdbc;

    public MybatisPlusHrPositionRepository(HrPositionMapper mapper,
                                           HrEmployeeMapper employeeMapper,
                                           java.time.Clock hrClock, JdbcHrOrganizationStore organization,
    ...
    public HrPositionView create(String tenantId, String positionCode,
        ...
        if (existsByName(tenantId, command.positionName(), null)) {
            throw conflict("岗位名称已存在");
        }
        ...
        try {
            ...
            throw conflict("岗位编号或名称已存在");
        }
        organization.change(tenantId, "POSITION_CREATE", row.positionCode, actorId);
        return position(tenantId, row.id).orElseThrow(() -> notFound("岗位不存在"));
    }
    ...
    public HrPositionView update(String tenantId, Long id,
        ...
        if (existsByName(tenantId, command.positionName(), existing.id)) {
            throw conflict("岗位名称已存在");
        }
        if (!existing.positionCode.equals(command.positionCode())) {
            if (referenceCount(tenantId, existing) > 0) throw conflict("岗位已被员工或任职记录引用，不能修改编码");
            if (existsByPositionCode(tenantId, command.positionCode())) throw conflict("岗位编码已存在");
        }
        LocalDateTime now = now();
        int updated = getBaseMapper().update(null, new UpdateWrapper<HrPositionEntity>()
                ...
                .set("position_code", command.positionCode())
        ...
        if (updated != 1) throw conflict("岗位已被其他流程修改，请刷新后重试");
        jdbc.update("UPDATE hr_employee SET primary_position_name_snapshot=?,revision=revision+1,updated_by=?,updated_by_name=NULL,updated_time=? WHERE tenant_id=? AND primary_position_code=? AND deleted=0 AND NOT(primary_position_name_snapshot <=> ?)",
                command.positionName(), actorId, now, tenantId, existing.positionCode, command.positionName());
        organization.change(tenantId, "POSITION_UPDATE", command.positionCode(), actorId);
        return position(tenantId, id).orElseThrow(() -> notFound("岗位不存在"));
    }
    ...
    public void delete(String tenantId, Long id, int revision, String actorId) {
        ...
        if (referenceCount > 0) {
            throw conflict("岗位已被员工主档引用，不能删除");
        }
        int updated = getBaseMapper().update(null, new UpdateWrapper<HrPositionEntity>()
                .eq("tenant_id", tenantId)
                .eq("id", id)
                ...
                .set("updated_time", now()));
        if (updated != 1) throw conflict("岗位已被其他流程修改，请刷新后重试");
        organization.change(tenantId, "POSITION_DELETE", existing.positionCode, actorId);
    }
    ...
    private QueryWrapper<HrPositionEntity> positionQuery(String tenantId, PositionSearchCriteria criteria) {
        PositionSearchCriteria c = criteria == null
                ? new PositionSearchCriteria(null, null, null)
                : criteria;
    ...
    private HrPositionEntity requireActive(String tenantId, Long id) {
        HrPositionEntity row = getBaseMapper().selectOne(new QueryWrapper<HrPositionEntity>()
                ...
                .eq("deleted", 0)
                .last("LIMIT 1"));
        if (row == null) throw notFound("岗位不存在");
        return row;
    }

    private boolean existsByName(String tenantId, String positionName, Long excludedId) {
        QueryWrapper<HrPositionEntity> query = new QueryWrapper<HrPositionEntity>()
                .eq("tenant_id", tenantId)
    ...
    private long referenceCount(String tenantId, HrPositionEntity position) {
        ...
        return employeeMapper.selectCount(query) + jdbc.queryForObject("SELECT COUNT(*) FROM hr_employee_assignment WHERE tenant_id=? AND position_code=?", Long.class, tenantId, position.positionCode);
    }
    ...
    private static HrPositionView view(HrPositionEntity row) {
        return new HrPositionView(row.id, row.positionCode, row.positionName,
                row.statusCode, row.remark, row.revision, row.createdBy,
                instant(row.createdTime), row.updatedBy, instant(row.updatedTime), row.sortOrder);
    }
